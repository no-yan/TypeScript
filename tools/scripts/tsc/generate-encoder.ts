...
function generateGoEncoder(): string {
    ...
    w.write("// Code generated by tools/scripts/tsc/generate-encoder.ts. DO NOT EDIT.");
    w.write("");
...
function emitRemoteNodeList(w: CodeWriter) {
    ...
    w.write(`    private _cursorNodeIndex: number = 0;`);
    w.write(``);
    ...
    w.write(`        // materializing every intermediate RemoteNode just to read it. Resume from`);
    w.write(`        // the memoized cursor when possible so sequential forward access is O(1)`);
    w.write(`        // amortized (a full in-order pass is O(n) rather than O(n^2)).`);
    w.write(`        const offsetNodes = this.sourceFile._offsetNodes;`);
    w.write(`        let i: number;`);
    w.write(`        let next: number;`);
    w.write(`        if (index >= this._cursorIndex) {`);
    w.write(`            i = this._cursorIndex;`);
    w.write(`            next = this._cursorNodeIndex;`);
    w.write(`        }`);
    w.write(`        else {`);
    w.write(`            i = 0;`);
    w.write(`            next = this.index + 1;`);
    w.write(`        }`);
    w.write(`        for (; i < index; i++) {`);
    w.write(`            next = this.view.getUint32(offsetNodes + next * NODE_LEN + NODE_OFFSET_NEXT, true);`);
    w.write(`        }`);
    w.write(`        this._cursorIndex = index;`);
    w.write(`        this._cursorNodeIndex = next;`);
    ...
    w.write(`            child = new RemoteNode(this.view, index, this.parent, sf, sf._offsetNodes);`);
    w.write(`            sf.nodes[index] = child;`);
    w.write(`            sf._timing?.recordMaterialization();`);
    w.write(`        }`);
...
function emitRemoteNodeClassOpen(w: CodeWriter) {
    ...
    w.write(`        if (!child) {`);
    w.write(`            const sf = this.sourceFile;`);
    w.write(`            const offsetNodes = sf._offsetNodes;`);
