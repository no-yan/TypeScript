...
function generateStoreForEachChild(): string {
    const w = new CodeWriter();
    w.write("// Code generated by tools/scripts/tsc/generate-go-ast.ts. DO NOT EDIT.");
    w.write("");
    w.write("package ast");
    w.write("");
    w.write("func (node Handle) forEachChildSchema(v StoreVisitor) bool {");
    w.push();
    w.write("switch node.Kind {");
    ...
    for (const node of api.nodes()) {
        ...
        if (kinds.length === 0) continue;
        for (const kind of kinds) emitted.add(kind);
        w.write(`case ${kinds.join(", ")}:`);
        w.push();
        if (node.handWrittenVisitor) {
            if (node.name !== "JSDocParameterOrPropertyTag") {
                throw new Error(`missing Store ForEachChild implementation for ${node.name}`);
            }
            w.write("return node.forEachChildJSDocParameterOrPropertyTag(v)");
            w.pop();
            continue;
        }
        for (const member of childMembers) {
            const getter = storeMemberGetter(node, member);
            if (member.listKind !== undefined) {
                w.write(`if node.forEachChildList(${getter}, v) { return true }`);
            }
            else {
                w.write(`if child := ${getter}; !child.IsNil() && v(child) { return true }`);
            }
        }
...
// Snapshot helpers return scalar references so callers need not retain storage pointers.
export function generateStoreSyntaxChildren(): string {
    const w = new CodeWriter();
    w.write("// Code generated by tools/scripts/tsc/generate-go-ast.ts. DO NOT EDIT.");
    w.write("package ast");
    const counts = [...new Set(Array.from(api.nodes(), node => storeLayout(node).children.length))].filter(n => n > 0).sort((a, b) => a - b);
    for (const count of counts) {
        const types = Array(count).fill("NodeRef").join(", ");
        w.write(`// SyntaxChildren${count} snapshots ${count} same-Store child slots of a caller-known shape.`);
        w.write("// Missing/foreign children are zero. Later syntax edits do not change returned refs.");
        w.write(`func (s *Store) SyntaxChildren${count}(ref NodeRef) (${types}) {`);
        w.write(`if s == nil || ref == 0 { return ${Array(count).fill("0").join(", ")} }`);
        w.write("n := &s.nodes[ref]");
        w.write(`if n.childLen != ${count} { panic("ast: unexpected syntax child count") }`);
        w.write("start := int(n.childStart)");
        w.write(`children := s.children[start:start+${count}]`);
        w.write(`return ${Array.from({ length: count }, (_, i) => `children[${i}]`).join(", ")}`);
        w.write("}");
    }
    return w.toString();
}

// Named Access* snapshots. Type/field aliases live only here; callers never see slot numbers.
const ACCESSOR_TYPE_ALIASES: Record<string, string> = {
    ParameterDeclaration: "Parameter",
};

const ACCESSOR_FIELD_ALIASES: Record<string, string> = {
    DotDotDotToken: "Rest",
...
function accessorTypeName(node: NodeType): string {
    return ACCESSOR_TYPE_ALIASES[node.name] ?? node.name;
}

function accessorFieldName(node: NodeType, m: MemberInfo): string {
    const raw = memberSuffix(m);
    return ACCESSOR_FIELD_ALIASES[`${node.name}.${raw}`] ?? ACCESSOR_FIELD_ALIASES[raw] ?? raw;
}

function assertAccessorApiName(name: string, kind: "type" | "method") {
    if (name === "Info" || name.startsWith("Walk") || name.startsWith("ChildrenOf")) {
        throw new Error(`reserved Access* ${kind} name: ${name}`);
    }
}

export function generateStoreAccessors(): string {
    const w = new CodeWriter();
    w.write("// Code generated by tools/scripts/tsc/generate-go-ast.ts. DO NOT EDIT.");
    w.write("package ast");
    w.write("");
    w.write("// Named Access* helpers snapshot same-Store child and list slots as integer refs.");
    w.write("// Returned structs hold NodeRef/ListRef only. They do not retain Store, Flags, Symbol, or Flow.");
    w.write("// Field reads do not touch the Store. Later syntax edits are not reflected in a snapshot.");
    w.write("");

    const usedTypeNames = new Map<string, string>();
    for (const node of api.nodes()) {
        const layout = storeLayout(node);
        if (layout.children.length === 0 && layout.lists.length === 0) {
            continue;
        }
        // Child-only projections avoid qualifying unused lists in syntax-only consumers.
        const projections = [false, ...(["BinaryExpression", "CallExpression"].includes(node.name) ? [true] : [])];
        for (const childrenOnly of projections) {
            const typeName = accessorTypeName(node) + (childrenOnly ? "Children" : "");
            assertAccessorApiName(typeName, "type");
            assertAccessorApiName(`Access${typeName}`, "method");
            const prior = usedTypeNames.get(typeName);
            if (prior !== undefined && prior !== node.name) {
                throw new Error(`Access* type name collision: ${typeName} for ${prior} and ${node.name}`);
            }
            usedTypeNames.set(typeName, node.name);

            const fieldNames = new Set<string>();
            const childFields = layout.children.map(m => {
                const name = accessorFieldName(node, m);
                if (fieldNames.has(name)) {
                    throw new Error(`Access* field collision on ${typeName}: ${name}`);
                }
                fieldNames.add(name);
                return { name, member: m };
            });
            const listFields = (childrenOnly ? [] : layout.lists).map(m => {
                const name = accessorFieldName(node, m);
                if (fieldNames.has(name)) {
                    throw new Error(`Access* field collision on ${typeName}: ${name}`);
                }
            ...
            w.write(`type ${typeName}Accessor struct {`);
            for (const f of childFields) {
                w.write(`${f.name} NodeRef`);
            }
            for (const f of listFields) {
                w.write(`${f.name} ListRef`);
            }
            w.write("}");
            w.write("");
            ...
            if (kinds.length === 0) {
                throw new Error(`Access${typeName}: no kinds for ${node.name}`);
            }
            const childCount = layout.children.length;
            const listCount = layout.lists.length;

            w.write(`func (s *Store) Access${typeName}(ref NodeRef) ${typeName}Accessor {`);
            w.write(`if s == nil || ref == 0 { return ${typeName}Accessor{} }`);
            w.write("n := &s.nodes[ref]");
            if (kinds.length === 1) {
                w.write(`if n.kind != ${kinds[0]} { panic("ast: Access${typeName}: unexpected kind") }`);
            }
            else {
                w.write("switch n.kind {");
                w.write(`case ${kinds.join(", ")}:`);
                w.write("default:");
                w.write(`panic("ast: Access${typeName}: unexpected kind")`);
                w.write("}");
            }
            w.write(`if n.childLen != ${childCount} || n.listLen != ${listCount} { panic("ast: Access${typeName}: unexpected schema shape") }`);
            w.write(`var a ${typeName}Accessor`);
            if (childCount > 0) {
                w.write("start := int(n.childStart)");
                w.write(`children := s.children[start:start+${childCount}]`);
                for (let i = 0; i < childFields.length; i++) {
                    w.write(`a.${childFields[i].name} = children[${i}]`);
                }
            }
            if (listFields.length > 0) {
                w.write("listBase := n.listBase()");
                for (let i = 0; i < listFields.length; i++) {
                    w.write(`a.${listFields[i].name} = s.listSlot(listBase + ${i})`);
                }
            }
            w.write("return a");
            w.write("}");
...
export function generateBinderWalk(): string {
    const w = new CodeWriter();
    w.write("// Code generated by tools/scripts/tsc/generate-go-ast.ts. DO NOT EDIT.");
    w.write("");
...
export default function main() {
    ...
    writeAndFormat(path.join(ROOT, "tsc/internal/ast/store_foreach_child_generated.go"), generateStoreForEachChild() + "\n");

    writeAndFormat(path.join(ROOT, "tsc/internal/ast/store_syntax_children_generated.go"), generateStoreSyntaxChildren() + "\n");

    writeAndFormat(path.join(ROOT, "tsc/internal/ast/store_accessors_generated.go"), generateStoreAccessors() + "\n");
