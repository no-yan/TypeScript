...
type DiagnosticAndArguments struct {
	...
	/*errorNode*/ // In non-strictNullChecks mode, `undefined` and `null` are assignable to anything except `never`.
	...
	/*enclosingDeclaration*/ // If we have 2 enums with *known* values that differ, they are incompatible.
	...
	/*reportErrors*/ // Record this relation as having failed such that we don't attempt the overflowing operation again.
	/*ignoreConstraints*/ // Check if we should issue an extra diagnostic to produce a quickfix for a slightly incorrect import statement
	/*errorNode*/ // Likely an incorrect import. Issue a helpful diagnostic to produce a quickfix to change the import
	/*errorNode*/ /*isCache*/ // Don't elaborate on indexes on generic variables
	/*errorNode*/ // Don't elaborate on indexes on generic variables or when types match
	/*headMessage*/ // Issue error on the prop itself, since the prop couldn't elaborate the error
	// Use the expression type, if available
	// Use the custom diagnostic factory if provided (e.g., for JSX text children with dynamic error messages)
	/*accessNode*/ // If for whatever reason the expression type doesn't yield an error, make sure we still issue an error on the sourcePropType
	/*isCache*/ // Don't elaborate blocks or functions with annotated parameter types
	/*errorNode*/ /*headMessage*/ /*headMessage*/ /*errorNode*/ // A type is 'weak' if it is an object type with at least one optional property
	...
	/*skipObjectFunctionPropertyAugment*/ // Return the name of a discriminant property for which it was possible and feasible to construct a map of
	...
	/*enclosingDeclaration*/ // Okay, yes, 'boolean' is a union of 'true | false', but that's not useful
	...
	/*reportErrors*/ /*errorReporter*/ /*reportUnreliableMarkers*/ // the second condition should be redundant, because there is no error reporting when comparing signatures by strict arity
	// since it is only done for subtype reduction
	/*inferenceContext*/ // void sources are assignable to anything.
	/*reportErrors*/ // In order to ensure that any generic type Foo<T> is at least co-variant with respect to T no matter
	...
	// with respect to T.
	/*reportErrors*/ // With strict arity, (x: number | undefined) => void is a subtype of (x?: number | undefined) => void
	/*reportErrors*/ // If a signature resolution is already in-flight, skip issuing a circularity error
	// here and just use the `any` type directly
	// The following block preserves behavior forbidding boolean returning functions from being assignable to type guard returning functions
	// When relating callback signatures, we still need to relate return types bi-variantly as otherwise
	// the containing type wouldn't be co-variant. For example, interface Foo<T> { add(cb: () => T): void }
	// wouldn't be co-variant for T without this rule.
	/*reportErrors*/ // The errors reported here serve as markers that trigger error chain reduction in the (*Relater).reportError
	...
	/*parameterName*/ /*parameterIndex*/ // We require an exact match for generic signatures, so we only return signatures from the first
	// signature list and only if they have exact matches in the other signature lists.
	/*partialMatch*/ /*ignoreThisTypes*/ /*ignoreReturnTypes*/ // Allow matching non-generic signatures to have excess parameters (as a fallback if exact parameter match is not found) and different return types.
	// Prefer matching this types if possible.
	/*partialMatch*/ /*ignoreThisTypes*/ /*ignoreReturnTypes*/ /*partialMatch*/ /*ignoreThisTypes*/ /*ignoreReturnTypes*/ /**
	 * See signatureRelatedTo, compareSignaturesIdentical
	 */ // Check that the two signatures have the same number of type parameters.
	// Check that type parameter constraints and defaults match. If they do, instantiate the source
	// signature with the type parameters of the target signature and continue the comparison.
	/*eraseTypeParameters*/ // A source signature matches a target signature if the two signatures have the same number of required,
	...
	/*writing*/ // Two template literal types with differences in their starting or ending text spans are definitely unrelated.
	...
	/*roundTripOnly*/ /*roundTripOnly*/ // We need to see whether applying the same mappings of the target
	...
	/*reportErrors*/ /*headMessage*/ // Before normalization: if `source` is type an object type, and `target` is primitive,
	// skip all the checks we don't need and just return `isSimpleTypeRelatedTo` result
	// Normalize the source and target types: Turn fresh literal types into regular literal types,
	// turn deferred type references into regular type references, simplify indexed access and
	// conditional types, and resolve substitution types to either the substitution (on the source
	// side) or the type variable (on the target side).
	/*writing*/ /*writing*/ /*reportErrors*/ // We fastpath comparing a type parameter to exactly its constraint, as this is _super_ common,
	...
	/*writing*/ /*reportErrors*/ /*reportErrors*/ // Disable excess property checks on JS literals to simulate having an implicit "index signature" - but only outside of noImplicitAny
	...
	// since `children` is a Kind.
	// PropertySignature instead of a Kind.JsxAttribute.
	...
	/*reportErrors*/ /*reportErrors*/ // Check to see if any constituents of the intersection are immediately related to the target.
	...
	/*reportErrors*/ /*headMessage*/ // We strip `undefined` from the target if the `source` trivially doesn't contain it for our correspondence-checking fastpath
	...
	/*reportErrors*/ /*headMessage*/ /*headMessage*/ // When relating a literal type to a union of primitive types, we know the relation is false unless
	...
	/*reportErrors*/ /*headMessage*/ /*reportErrors*/ /*headMessage*/ // Elaborate only if we can find a best matching type in the target union
	/*reportErrors*/ /*headMessage*/ /*headMessage*/ /*reportErrors*/ // Determine if possibly recursive types are related. First, check if the result is already available in the global cache.
	...
	/*ignoreConstraints*/ // We are elaborating errors and the cached result is a failure not due to a comparison overflow,
	...
	// were unconstrained.
	/*ignoreConstraints*/ // We stop relating if we reach 100 levels of nesting. This is a backstop to catch infinite recursion
	...
	/*reportErrors*/ /*headMessage*/ // When the target is an intersection we need an extra property check in order to detect nested excess
	...
	//
	/*excludedProperties*/ /*optionalsOnly*/ /*sourceIsPrimitive*/ // When the source is an intersection we need an extra check of any optional properties in the target to
	...
	//   }
	//
	/*excludedProperties*/ /*optionalsOnly*/ // If some type parameter was `Unmeasurable` or `Unreliable`, and we couldn't pass by assuming it was identical, then we
	...
	/*reportErrors*/ /*reportErrors*/ /*reportErrors*/ /*reportErrors*/ /*reportErrors*/ /*reportErrors*/ /*reportErrors*/ /*reportErrors*/ /*reportErrors*/ /*reportErrors*/ /*reportErrors*/ // The ordered decomposition above doesn't handle all cases. Specifically, we also need to handle:
	...
	/*reportErrors*/ /*reportErrors*/ // A source type { [P in Q]: X } is related to a target type T if keyof T is related to Q and X is related to T[Q].
	...
	/*reportErrors*/ // Relate components directly before falling back to constraint relationships
	// A type S[K] is related to a type T[J] if S is related to T and K is related to J.
	// A type S is related to a type T[K] if S is related to C, where C is the base
	// constraint of T[K] for writing.
	// create a new chain for the constraint error
	/*headMessage*/ // prefer the shorter chain of the constraint comparison chain, and the direct comparison chain
	// A keyof S is related to a keyof T if T is related to S.
	/*reportErrors*/ // An index type can have a tuple type target when the tuple type contains variadic elements.
	...
	/*reportErrors*/ /*headMessage*/ /*reportErrors*/ /*headMessage*/ // Report unreliable variance for type variables referenced in template literal type placeholders.
	...
	/*reportErrors*/ /*headMessage*/ /*needApparentType*/ /*headMessage*/ // For an indexed access type { [P in K]: E}[X], above we have already explored an instantiation of E with X
	...
	/*signature*/ // conditionals can be related to one another via normal constraint, as, eg, `A extends B ? O : never` should be assignable to `O`
	...
	/*excludedProperties*/ /*optionalsOnly*/ // Use variance error (there is no structural one) and return false
	...
	/*reportErrors*/ // Propagate unreliable variance flag in variance computations
	/*headMessage*/ /*headMessage*/ // In the bivariant case we first compare contravariantly without reporting
	// errors. Then, if that doesn't succeed, we compare covariantly with error
	// reporting. Thus, error elaboration will be based on the covariant check,
	// which is generally easier to reason about.
	/*reportErrors*/ /*headMessage*/ // In the invariant case we first compare covariantly, and only when that
	// succeeds do we proceed to compare contravariantly. Thus, error elaboration
	// will typically be based on the covariant check.
	/*headMessage*/ /*headMessage*/ // A type [P in S]: X is related to a type [Q in T]: Y if T is related to S and X' is
	...
	/*reportErrors*/ /*skipOptional*/ // If the target property could not be found, or if the properties were not related,
	// then this constituent is not a match.
	// We failed to match any type for this combination.
	// Compare the remaining non-discriminant properties of each match.
	/*reportErrors*/ /*optionalsOnly*/ /*reportErrors*/ /*reportErrors*/ // Comparing numeric index types when both `source` and `type` are tuples is unnecessary as the
	...
	// in the index type.
	/*sourceIsPrimitive*/ /*reportErrors*/ // We can only exclude discriminant properties if we have not yet encountered a variable-length element.
	/*headMessage*/ /*matchDiscriminantProperties*/ // We only call this for union target types when we're attempting to do excess property checking - in those cases, we want to get _all possible props_
	...
	// (M - property in T)
	// (N - property in S)
	/*isProperty*/ // source could resolve to `any` and that's not related to `unknown` target under strict subtype relation
	/*headMessage*/ // give specific error in case where private names have the same description
	/*matchDiscriminantProperties*/ /*reportErrors*/ /**
	...
	/*erase*/ // For simple functions (functions with a single signature) we only erase type parameters for
	// the comparable relation. Otherwise, if the source signature is generic, we instantiate it
	// in the context of the target signature before checking the relationship. Ideally we'd do
	// this regardless of the number of signatures, but the potential costs are prohibitive due
	// to the quadratic nature of the logic below.
	// Only elaborate errors from the first failure
	/*erase*/ // A public, protected and private signature is assignable to a private signature.
	// A public and protected signature is assignable to a protected signature.
	// Only a public signature is assignable to public signature.
	// See signatureAssignableTo, compareSignaturesIdentical
	/*headMessage*/ /*partialMatch*/ /*ignoreThisTypes*/ /*ignoreReturnTypes*/ // Intersection constituents are never considered to have an inferred index signature. Also, in the strict subtype relation,
	...
	/*headMessage*/ /*headMessage*/ /*reportErrors*/ /*enclosingDeclaration*/ // Don't generalize on 'never' - we really want the original type
...
type RecursionId struct{ value any }

func asRecursionId[T ast.Handle | *ast.Symbol | *Type](value T) RecursionId {
...
func (c *Checker) isTypeRelatedTo(source *Type, target *Type, relation *Relation) bool {
	...
	if source.flags&TypeFlagsStructuredOrInstantiable != 0 || target.flags&TypeFlagsStructuredOrInstantiable != 0 {
		return c.checkTypeRelatedTo(source, target, relation, ast.Handle{})
	}
	return false
}
		...
		if sourceProperty.Flags&ast.SymbolFlagsEnumMember != 0 {
			...
			if targetProperty == nil || targetProperty.Flags&ast.SymbolFlagsEnumMember == 0 {
				if errorReporter != nil {
					errorReporter(diagnostics.Property_0_is_missing_in_type_1, c.symbolToString(sourceProperty), c.TypeToStringEx(c.getDeclaredTypeOfSymbol(targetSymbol), ast.Handle{}, TypeFormatFlagsUseFullyQualifiedType, nil))
				}
...
func (c *Checker) checkTypeAssignableTo(source *Type, target *Type, errorNode ast.Handle, headMessage *diagnostics.Message) bool {
	return c.checkTypeRelatedToEx(source, target, c.assignableRelation, errorNode, headMessage, nil)
}
func (c *Checker) checkTypeAssignableToEx(source *Type, target *Type, errorNode ast.Handle, headMessage *diagnostics.Message, diagnosticOutput *[]*ast.Diagnostic) bool {
	return c.checkTypeRelatedToEx(source, target, c.assignableRelation, errorNode, headMessage, diagnosticOutput)
}
func (c *Checker) checkTypeComparableTo(source *Type, target *Type, errorNode ast.Handle, headMessage *diagnostics.Message) bool {
	return c.checkTypeRelatedToEx(source, target, c.comparableRelation, errorNode, headMessage, nil)
}
func (c *Checker) checkTypeRelatedTo(source *Type, target *Type, relation *Relation, errorNode ast.Handle) bool {
	return c.checkTypeRelatedToEx(source, target, relation, errorNode, nil, nil)
}

func (c *Checker) checkTypeRelatedToEx(source *Type, target *Type, relation *Relation, errorNode ast.Handle, headMessage *diagnostics.Message, diagnosticOutput *[]*ast.Diagnostic) bool {
	r := c.getRelater()
	r.relation = relation
	r.errorNode = errorNode
	r.relationCount = (16_000_000 - relation.size()) / 8
	result := r.isRelatedToEx(source, target, RecursionFlagsBoth, !errorNode.IsNil(), headMessage, IntersectionStateNone)
	if r.overflow {
		id, _ := getRelationKey(source, target, IntersectionStateNone, relation == c.identityRelation, false)
		...
		if headMessage != nil && !errorNode.IsNil() && result == TernaryFalse && source.symbol != nil && c.exportTypeLinks.Has(source.symbol) {
			...
			if !links.originatingImport.IsNil() && !ast.IsImportCall(links.originatingImport) {
				helpfulRetry := c.checkTypeRelatedTo(c.getTypeOfSymbol(links.target), target, relation, ast.Handle{})
				if helpfulRetry {
...
func createDiagnosticChainFromErrorChain(chain *ErrorChain, errorNode ast.Handle, relatedInfo []*ast.Diagnostic) *ast.Diagnostic {
...
func (c *Checker) checkTypeAssignableToAndOptionallyElaborate(source *Type, target *Type, errorNode ast.Handle, expr ast.Handle, headMessage *diagnostics.Message, diagnosticOutput *[]*ast.Diagnostic) bool {
...
func (c *Checker) checkTypeRelatedToAndOptionallyElaborate(source *Type, target *Type, relation *Relation, errorNode ast.Handle, expr ast.Handle, headMessage *diagnostics.Message, diagnosticOutput *[]*ast.Diagnostic) bool {
...
func (c *Checker) elaborateError(node ast.Handle, source *Type, target *Type, relation *Relation, headMessage *diagnostics.Message, diagnosticOutput *[]*ast.Diagnostic) bool {
...
func (c *Checker) elaborateDidYouMeanToCallOrConstruct(node ast.Handle, source *Type, target *Type, relation *Relation, kind SignatureKind, headMessage *diagnostics.Message, diagnosticOutput *[]*ast.Diagnostic) bool {
	if core.Some(c.getSignaturesOfType(source, kind), func(s *Signature) bool {
		returnType := c.getReturnTypeOfSignature(s)
		return returnType.flags&(TypeFlagsAny|TypeFlagsNever) == 0 && c.checkTypeRelatedTo(returnType, target, relation, ast.Handle{})
	}) {
		var diags []*ast.Diagnostic
		if !c.checkTypeRelatedToEx(source, target, relation, node, headMessage, &diags) {
			diagnostic := diags[0]
			message := core.IfElse(kind == SignatureKindConstruct, diagnostics.Did_you_mean_to_use_new_with_this_expression, diagnostics.Did_you_mean_to_call_this_expression)
...
func (c *Checker) elaborateObjectLiteral(node ast.Handle, source *Type, target *Type, relation *Relation, diagnosticOutput *[]*ast.Diagnostic) bool {
...
func (c *Checker) elaborateArrayLiteral(node ast.Handle, source *Type, target *Type, relation *Relation, diagnosticOutput *[]*ast.Diagnostic) bool {
	if target.flags&(TypeFlagsPrimitive|TypeFlagsNever) != 0 {
		return false
	}
	if !c.isTupleLikeType(source) {
		c.pushContextualType(node, target, false)
...
func (c *Checker) elaborateElement(source *Type, target *Type, relation *Relation, prop ast.Handle, next ast.Handle, nameType *Type, errorMessage *diagnostics.Message, diagnosticFactory func(prop ast.Handle) *ast.Diagnostic, diagnosticOutput *[]*ast.Diagnostic) bool {
	...
	if sourcePropType == nil || c.checkTypeRelatedTo(sourcePropType, targetPropType, relation, ast.Handle{}) {
		return false
	}
	if !next.IsNil() && c.elaborateError(next, sourcePropType, targetPropType, relation, nil, diagnosticOutput) {
		return true
	}
	var diags []*ast.Diagnostic
	...
	if targetProp == nil {
		...
		if indexInfo != nil && !indexInfo.declaration.IsNil() && !c.program.IsSourceFileDefaultLibrary(ast.GetSourceFileOfNode(indexInfo.declaration).Path()) {
...
func (c *Checker) getBestMatchIndexedAccessTypeOrUndefined(source *Type, target *Type, nameType *Type) *Type {
	idx := c.getIndexedAccessTypeOrUndefined(target, nameType, AccessFlagsNone, ast.Handle{}, nil)
	if idx != nil {
		return idx
	}
	if target.flags&TypeFlagsUnion != 0 {
		...
		if best != nil {
			return c.getIndexedAccessTypeOrUndefined(best, nameType, AccessFlagsNone, ast.Handle{}, nil)
		}
	}
	return nil
}
func (c *Checker) checkExpressionForMutableLocationWithContextualType(next ast.Handle, sourcePropType *Type) *Type {
...
func (c *Checker) elaborateArrowFunction(node ast.Handle, source *Type, target *Type, relation *Relation, diagnosticOutput *[]*ast.Diagnostic) bool {
	...
	if c.checkTypeRelatedTo(sourceReturn, targetReturn, relation, ast.Handle{}) {
		return false
	}
	if !returnExpression.IsNil() && c.elaborateError(returnExpression, sourceReturn, targetReturn, relation, nil, diagnosticOutput) {
		return true
	}
	var diags []*ast.Diagnostic
	c.checkTypeRelatedToEx(sourceReturn, targetReturn, relation, returnExpression, nil, &diags)
	if len(diags) != 0 {
		diagnostic := diags[0]
		...
		if ast.GetFunctionFlags(node)&ast.FunctionFlagsAsync == 0 && c.getTypeOfPropertyOfType(sourceReturn, "then") == nil && c.checkTypeRelatedTo(c.createPromiseType(sourceReturn), targetReturn, relation, ast.Handle{}) {
...
func (c *Checker) isKnownProperty(targetType *Type, name string, isComparingJsxAttributes bool) bool {
	if targetType.flags&TypeFlagsObject != 0 {
		if c.getPropertyOfObjectType(targetType, name) != nil || c.getApplicableIndexInfoForName(targetType, name) != nil || isLateBoundName(name) && c.getIndexInfoOfType(targetType, c.stringType) != nil || isComparingJsxAttributes && isHyphenatedJsxName(name) {
			return true
		}
	}
...
func isExcessPropertyCheckTarget(t *Type) bool {
	return t.flags&TypeFlagsObject != 0 && t.objectFlags&ObjectFlagsObjectLiteralPatternWithComputedProperties == 0 || t.flags&TypeFlagsNonPrimitive != 0 || t.flags&TypeFlagsSubstitution != 0 && isExcessPropertyCheckTarget(t.AsSubstitutionType().baseType) || t.flags&TypeFlagsUnion != 0 && core.Some(t.Types(), isExcessPropertyCheckTarget) || t.flags&TypeFlagsIntersection != 0 && core.Every(t.Types(), isExcessPropertyCheckTarget)
}
...
func getRecursionIdentityTarget(t *Type) *Type {
	...
	if t.objectFlags&ObjectFlagsInstantiatedMapped == ObjectFlagsInstantiatedMapped {
		...
		if target != nil && (target.symbol != nil || target.flags&TypeFlagsIntersection != 0 && core.Some(target.Types(), func(t *Type) bool {
			return t.symbol != nil
		})) {
...
func (c *Checker) findBestTypeForInvokable(source *Type, unionTarget *Type, kind SignatureKind) *Type {
	if len(c.getSignaturesOfType(source, kind)) != 0 {
		return core.Find(unionTarget.Types(), func(t *Type) bool {
			return len(c.getSignaturesOfType(t, kind)) != 0
		})
	}
	return nil
}
...
func (c *Checker) shouldReportUnmatchedPropertyError(source *Type, target *Type) bool {
	...
	if (len(typeCallSignatures) != 0 || len(typeConstructSignatures) != 0) && len(typeProperties) == 0 {
		if (len(c.getSignaturesOfType(target, SignatureKindCall)) != 0 && len(typeCallSignatures) != 0) || len(c.getSignaturesOfType(target, SignatureKindConstruct)) != 0 && len(typeConstructSignatures) != 0 {
			return true
		}
		return false
	}
...
func (c *Checker) isDiscriminantProperty(t *Type, name string) bool {
	if t != nil && t.flags&TypeFlagsUnion != 0 {
		prop := c.getUnionOrIntersectionProperty(t, name, false)
...
func (c *Checker) getTypeNameForErrorDisplay(t *Type) string {
	return c.typeToStringEx(t, ast.Handle{}, TypeFormatFlagsUseFullyQualifiedType, nil)
}
		...
		if stackIndex < 0 {
			...
			for i, tp := range typeParameters {
				...
				default:
					...
					variance = core.IfElse(c.isTypeAssignableTo(typeWithSub, typeWithSuper), VarianceFlagsCovariant, 0) | core.IfElse(c.isTypeAssignableTo(typeWithSuper, typeWithSub), VarianceFlagsContravariant, 0)
...
func (c *Checker) isSignatureAssignableTo(source *Signature, target *Signature, ignoreReturnTypes bool) bool {
	return c.compareSignaturesRelated(source, target, core.IfElse(ignoreReturnTypes, SignatureCheckModeIgnoreReturnTypes, SignatureCheckModeNone), false, nil, c.compareTypesAssignable, nil) != TernaryFalse
}
func (c *Checker) compareSignaturesRelated(source *Signature, target *Signature, checkMode SignatureCheckMode, reportErrors bool, errorReporter ErrorReporter, compareTypes TypeComparer, reportUnreliableMarkers *TypeMapper) Ternary {
	...
	if len(source.typeParameters) != 0 && !core.Same(source.typeParameters, target.typeParameters) {
		...
		source = c.instantiateSignatureInContextOf(source, target, nil, compareTypes)
	}
	...
	if sourceThisType != nil && sourceThisType != c.voidType {
		...
		if targetThisType != nil {
			var related Ternary
			if !strictVariance {
				related = compareTypes(sourceThisType, targetThisType, false)
			}
	...
	for i := range paramCount {
		...
		if sourceType != nil && targetType != nil && (sourceType != targetType || checkMode&SignatureCheckModeStrictArity != 0) {
			...
			callbacks := sourceSig != nil && targetSig != nil && c.getTypePredicateOfSignature(sourceSig) == nil && c.getTypePredicateOfSignature(targetSig) == nil && c.getTypeFacts(sourceType, TypeFactsIsUndefinedOrNull) == c.getTypeFacts(targetType, TypeFactsIsUndefinedOrNull)
			var related Ternary
			if callbacks {
				...
				if checkMode&SignatureCheckModeCallback == 0 && !strictVariance {
					related = compareTypes(sourceType, targetType, false)
				}
				if related == TernaryFalse {
					related = compareTypes(targetType, sourceType, reportErrors)
				}
			}
			if related != TernaryFalse && checkMode&SignatureCheckModeStrictArity != 0 && i >= c.getMinArgumentCount(source) && i < c.getMinArgumentCount(target) && compareTypes(sourceType, targetType, false) != TernaryFalse {
	...
	if checkMode&SignatureCheckModeIgnoreReturnTypes == 0 {
		...
		if targetTypePredicate != nil {
			...
			if checkMode&SignatureCheckModeBivariantCallback != 0 {
				related = compareTypes(targetReturnType, sourceReturnType, false)
			}
			if related == TernaryFalse {
			...
			if result == TernaryFalse && reportErrors {
				...
				if len(source.parameters) == 0 && len(target.parameters) == 0 {
					message = core.IfElse(source.flags&SignatureFlagsConstruct != 0, diagnostics.Construct_signatures_with_no_arguments_have_incompatible_return_types_0_and_1, diagnostics.Call_signatures_with_no_arguments_have_incompatible_return_types_0_and_1)
				} else {
					message = core.IfElse(source.flags&SignatureFlagsConstruct != 0, diagnostics.Construct_signature_return_types_0_and_1_are_incompatible, diagnostics.Call_signature_return_types_0_and_1_are_incompatible)
				}
...
func (c *Checker) getNameableDeclarationAtPosition(signature *Signature, pos int) ast.Handle {
	paramCount := len(signature.parameters) - core.IfElse(signatureHasRestParameter(signature), 1, 0)
	if pos < paramCount {
...
func (c *Checker) isValidDeclarationForTupleLabel(d ast.Handle) bool {
...
func (c *Checker) sliceTupleType(t *Type, index int, endSkipCount int) *Type {
	...
	return c.createTupleTypeEx(c.getTypeArguments(t)[index:endIndex], target.elementInfos[index:endIndex], false)
}
...
func (c *Checker) createTypePredicateFromTypePredicateNode(node ast.Handle, signature *Signature) *TypePredicate {
	predicateNode := node
	var t *Type
	...
	if ast.IsThisTypeNode(predicateNode.ParameterName()) {
		...
		return c.newTypePredicate(kind, "", 0, t)
	}
	...
	index := core.FindIndex(signature.parameters, func(p *ast.Symbol) bool {
		return p.Name == name
	})
...
func (c *Checker) findMatchingSignatures(signatureLists [][]*Signature, signature *Signature, listIndex int) []*Signature {
	if len(signature.typeParameters) != 0 {
		...
		for i := 1; i < len(signatureLists); i++ {
			if c.findMatchingSignature(signatureLists[i], signature, false, false, false) == nil {
				return nil
			}
		}
	...
	for i := range signatureLists {
		...
		if i == listIndex {
			...
			match = c.findMatchingSignature(signatureLists[i], signature, false, false, true)
			if match == nil {
				match = c.findMatchingSignature(signatureLists[i], signature, true, false, true)
			}
		}
		if match == nil {
			return nil
...
func (c *Checker) compareSignaturesIdentical(source *Signature, target *Signature, partialMatch bool, ignoreThisTypes bool, ignoreReturnTypes bool, compareTypes func(s *Type, t *Type) Ternary) Ternary {
	...
	if len(target.typeParameters) != 0 {
		...
		for i := range len(target.typeParameters) {
			...
			if !(s == t || compareTypes(c.instantiateType(c.getConstraintOrUnknownFromTypeParameter(s), mapper), c.getConstraintOrUnknownFromTypeParameter(t)) != TernaryFalse && compareTypes(c.instantiateType(c.getDefaultOrUnknownFromTypeParameter(s), mapper), c.getDefaultOrUnknownFromTypeParameter(t)) != TernaryFalse) {
				return TernaryFalse
			}
		}
		source = c.instantiateSignatureEx(source, mapper, true)
	}
	result := TernaryTrue
	if !ignoreThisTypes {
		sourceThisType := c.getThisTypeOfSignature(source)
...
func (c *Checker) getEffectiveConstraintOfIntersection(types []*Type, targetIsUnion bool) *Type {
	...
	if constraints != nil && (targetIsUnion || hasDisjointDomainType) {
		...
		return c.getNormalizedType(c.getIntersectionTypeEx(constraints, IntersectionFlagsNoConstraintReduction, nil), false)
	}
	return nil
}
...
func (c *Checker) isValidTypeForTemplateLiteralPlaceholder(source *Type, target *Type, compareTypes TypeComparer) bool {
	...
	case source.flags&TypeFlagsStringLiteral != 0:
		...
		return target.flags&TypeFlagsNumber != 0 && isValidNumberString(value, false) || target.flags&TypeFlagsBigInt != 0 && isValidBigIntString(value, false) || target.flags&(TypeFlagsBooleanLiteral|TypeFlagsNullable) != 0 && value == target.AsIntrinsicType().intrinsicName || target.flags&TypeFlagsStringMapping != 0 && c.isMemberOfStringMapping(source, target) || target.flags&TypeFlagsTemplateLiteral != 0 && c.isTypeMatchedByTemplateLiteralType(source, target.AsTemplateLiteralType(), compareTypes)
	case source.flags&TypeFlagsTemplateLiteral != 0:
...
func (c *Checker) putRelater(r *Relater) {
	...
	*r = Relater{c: c, maybeKeys: r.maybeKeys[:0], maybeKeysSet: r.maybeKeysSet, sourceStack: r.sourceStack[:0], targetStack: r.targetStack[:0], next: c.freeRelater}
	c.freeRelater = r
}
func (r *Relater) isRelatedToSimple(source *Type, target *Type) Ternary {
	return r.isRelatedToEx(source, target, RecursionFlagsBoth, false, nil, IntersectionStateNone)
}
...
func (r *Relater) isRelatedToEx(originalSource *Type, originalTarget *Type, recursionFlags RecursionFlags, reportErrors bool, headMessage *diagnostics.Message, intersectionState IntersectionState) Ternary {
	...
	if originalSource.flags&TypeFlagsObject != 0 && originalTarget.flags&TypeFlagsPrimitive != 0 {
		if r.relation == r.c.comparableRelation && originalTarget.flags&TypeFlagsNever == 0 && r.c.isSimpleTypeRelatedTo(originalTarget, originalSource, r.relation, nil) || r.c.isSimpleTypeRelatedTo(originalSource, originalTarget, r.relation, core.IfElse(reportErrors, r.reportError, nil)) {
			return TernaryTrue
		}
		if reportErrors {
	...
	source := r.c.getNormalizedType(originalSource, false)
	target := r.c.getNormalizedType(originalTarget, true)
	if source == target {
		return TernaryTrue
	}
	if r.relation == r.c.identityRelation {
		...
		return r.recursiveTypeRelatedTo(source, target, false, IntersectionStateNone, recursionFlags)
	}
	...
	if source.flags&TypeFlagsDefinitelyNonNullable != 0 && target.flags&TypeFlagsUnion != 0 {
		...
		if candidate != nil && candidate.flags&TypeFlagsNullable == 0 {
			target = r.c.getNormalizedType(candidate, true)
			if source == target {
				return TernaryTrue
			}
		}
	}
	if r.relation == r.c.comparableRelation && target.flags&TypeFlagsNever == 0 && r.c.isSimpleTypeRelatedTo(target, source, r.relation, nil) || r.c.isSimpleTypeRelatedTo(source, target, r.relation, core.IfElse(reportErrors, r.reportError, nil)) {
		return TernaryTrue
	}
	if source.flags&TypeFlagsStructuredOrInstantiable != 0 || target.flags&TypeFlagsStructuredOrInstantiable != 0 {
		...
		isPerformingCommonPropertyChecks := (r.relation != r.c.comparableRelation || isUnitType(source)) && intersectionState&IntersectionStateTarget == 0 && source.flags&(TypeFlagsPrimitive|TypeFlagsObject|TypeFlagsIntersection) != 0 && source != r.c.globalObjectType && target.flags&(TypeFlagsObject|TypeFlagsIntersection) != 0 && r.c.isWeakType(target) && (len(r.c.getPropertiesOfType(source)) > 0 || r.c.typeHasCallOrConstructSignatures(source))
		...
		if isPerformingCommonPropertyChecks && !r.c.hasCommonProperties(source, target, isComparingJsxAttributes) {
			if reportErrors {
				...
				if len(calls) > 0 && r.isRelatedTo(r.c.getReturnTypeOfSignature(calls[0]), target, RecursionFlagsSource, false) != TernaryFalse || len(constructs) > 0 && r.isRelatedTo(r.c.getReturnTypeOfSignature(constructs[0]), target, RecursionFlagsSource, false) != TernaryFalse {
		...
		r.traceUnionsOrIntersectionsTooLarge(source, target)
		skipCaching := source.flags&TypeFlagsUnion != 0 && len(source.Types()) < 4 && target.flags&TypeFlagsUnion == 0 || target.flags&TypeFlagsUnion != 0 && len(target.Types()) < 4 && source.flags&TypeFlagsStructuredOrInstantiable == 0
		var result Ternary
		if skipCaching {
			...
			if !r.c.isKnownProperty(reducedTarget, prop.Name, isComparingJsxAttributes) {
				if reportErrors {
					...
					if ast.IsJsxAttributes(r.errorNode) || ast.IsJsxOpeningLikeElement(r.errorNode) || ast.IsJsxOpeningLikeElement(r.errorNode.Parent()) {
						...
						if !prop.ValueDeclaration.IsNil() && ast.IsObjectLiteralElement(prop.ValueDeclaration) && !ast.FindAncestor(prop.ValueDeclaration, func(d ast.Handle) bool {
							return d == objectLiteralDeclaration
						}).IsNil() && ast.GetSourceFileOfNode(objectLiteralDeclaration) == ast.GetSourceFileOfNode(r.errorNode) {
	...
	if r.relation == r.c.comparableRelation && target.flags&TypeFlagsPrimitive != 0 {
		...
		if !core.Same(constraints, source.Types()) {
			...
			if source.flags&TypeFlagsIntersection == 0 {
				result := r.isRelatedTo(source, target, RecursionFlagsSource, false)
				if result != TernaryFalse {
					return result
				}
				return r.isRelatedTo(target, source, RecursionFlagsSource, false)
			}
		}
	}
	return r.someTypeRelatedToType(source, target, false, IntersectionStateSource)
}
func (r *Relater) someTypeRelatedToType(source *Type, target *Type, reportErrors bool, intersectionState IntersectionState) Ternary {
	...
	for i, t := range sourceTypes {
		related := r.isRelatedToEx(t, target, RecursionFlagsSource, reportErrors && i == len(sourceTypes)-1, nil, intersectionState)
		if related != TernaryFalse {
			return related
		}
	}
...
func (r *Relater) eachTypeRelatedToType(source *Type, target *Type, reportErrors bool, intersectionState IntersectionState) Ternary {
	...
	for i, sourceType := range sourceTypes {
		if strippedTarget.flags&TypeFlagsUnion != 0 && len(sourceTypes) >= len(strippedTypes) && len(sourceTypes)%len(strippedTypes) == 0 {
			related := r.isRelatedToEx(sourceType, strippedTypes[i%len(strippedTypes)], RecursionFlagsBoth, false, nil, intersectionState)
			if related != TernaryFalse {
				result &= related
				continue
			}
		}
		related := r.isRelatedToEx(sourceType, target, RecursionFlagsSource, reportErrors, nil, intersectionState)
		if related == TernaryFalse {
			return TernaryFalse
		}
		result &= related
...
func (r *Relater) typeRelatedToSomeType(source *Type, target *Type, reportErrors bool, intersectionState IntersectionState) Ternary {
	...
	if target.flags&TypeFlagsUnion != 0 {
		...
		if r.relation != r.c.comparableRelation && target.objectFlags&ObjectFlagsPrimitiveUnion != 0 && source.flags&TypeFlagsEnumLiteral == 0 && (source.flags&(TypeFlagsStringLiteral|TypeFlagsBooleanLiteral|TypeFlagsBigIntLiteral) != 0 || (r.relation == r.c.subtypeRelation || r.relation == r.c.strictSubtypeRelation) && source.flags&TypeFlagsNumberLiteral != 0) {
		...
		if match != nil {
			related := r.isRelatedToEx(source, match, RecursionFlagsTarget, false, nil, intersectionState)
			if related != TernaryFalse {
				return related
			}
		}
	...
	for _, t := range targetTypes {
		related := r.isRelatedToEx(source, t, RecursionFlagsTarget, false, nil, intersectionState)
		if related != TernaryFalse {
			return related
		}
	}
	if reportErrors {
		...
		if bestMatchingType != nil {
			r.isRelatedToEx(source, bestMatchingType, RecursionFlagsTarget, true, nil, intersectionState)
		}
	}
	return TernaryFalse
}
func (r *Relater) typeRelatedToEachType(source *Type, target *Type, reportErrors bool, intersectionState IntersectionState) Ternary {
	...
	for _, targetType := range targetTypes {
		related := r.isRelatedToEx(source, targetType, RecursionFlagsTarget, reportErrors, nil, intersectionState)
		if related == TernaryFalse {
			return TernaryFalse
		}
		result &= related
...
func (r *Relater) eachTypeRelatedToSomeType(source *Type, target *Type) Ternary {
	...
	for _, sourceType := range sourceTypes {
		related := r.typeRelatedToSomeType(sourceType, target, false, IntersectionStateNone)
		if related == TernaryFalse {
			return TernaryFalse
		}
		result &= related
...
func (r *Relater) recursiveTypeRelatedTo(source *Type, target *Type, reportErrors bool, intersectionState IntersectionState, recursionFlags RecursionFlags) Ternary {
	...
	id, constrained := getRelationKey(source, target, intersectionState, r.relation == r.c.identityRelation, false)
	...
	if constrained {
		broadestEquivalentId, _ := getRelationKey(source, target, intersectionState, r.relation == r.c.identityRelation, true)
...
func (r *Relater) getErrorState() errorState {
	return errorState{errorChain: r.errorChain, relatedInfo: r.relatedInfo}
}
...
func (r *Relater) structuredTypeRelatedTo(source *Type, target *Type, reportErrors bool, intersectionState IntersectionState) Ternary {
	...
	if r.relation != r.c.identityRelation {
		if result == TernaryFalse && (source.flags&TypeFlagsIntersection != 0 || source.flags&TypeFlagsTypeParameter != 0 && target.flags&TypeFlagsUnion != 0) {
			...
			if constraint != nil && everyType(constraint, func(c *Type) bool {
				return c != source
			}) {
				result = r.isRelatedToEx(constraint, target, RecursionFlagsSource, false, nil, intersectionState)
			}
		}
		switch {
		case result != TernaryFalse && intersectionState&IntersectionStateTarget == 0 && target.flags&TypeFlagsIntersection != 0 && !r.c.isGenericObjectType(target) && source.flags&(TypeFlagsObject|TypeFlagsIntersection) != 0:
			result &= r.propertiesRelatedTo(source, target, reportErrors, collections.Set[string]{}, false, IntersectionStateNone)
			if result != 0 && isObjectLiteralType(source) && source.objectFlags&ObjectFlagsFreshLiteral != 0 {
				result &= r.indexSignaturesRelatedTo(source, target, false, reportErrors, IntersectionStateNone)
			}
		case result != 0 && r.c.isNonGenericObjectType(target) && !r.c.isArrayOrTupleType(target) && r.isSourceIntersectionNeedingExtraCheck(source, target):
			result &= r.propertiesRelatedTo(source, target, reportErrors, collections.Set[string]{}, true, intersectionState)
		}
	}
	if result != TernaryFalse {
...
func (r *Relater) isSourceIntersectionNeedingExtraCheck(source *Type, target *Type) bool {
	return source.flags&TypeFlagsIntersection != 0 && r.c.getApparentType(source).flags&TypeFlagsStructuredType != 0 && !core.Some(source.Types(), func(t *Type) bool {
...
func (r *Relater) structuredTypeRelatedToWorker(source *Type, target *Type, reportErrors bool, intersectionState IntersectionState) Ternary {
	...
	case r.relation == r.c.identityRelation:
		...
		case source.flags&TypeFlagsIndex != 0:
			return r.isRelatedTo(source.Target(), target.Target(), RecursionFlagsBoth, false)
		case source.flags&TypeFlagsIndexedAccess != 0:
			result = r.isRelatedTo(source.AsIndexedAccessType().objectType, target.AsIndexedAccessType().objectType, RecursionFlagsBoth, false)
			if result != TernaryFalse {
				result &= r.isRelatedTo(source.AsIndexedAccessType().indexType, target.AsIndexedAccessType().indexType, RecursionFlagsBoth, false)
				if result != TernaryFalse {
					return result
				}
			}
		case source.flags&TypeFlagsConditional != 0:
			if source.AsConditionalType().root.isDistributive == target.AsConditionalType().root.isDistributive {
				result = r.isRelatedTo(source.AsConditionalType().checkType, target.AsConditionalType().checkType, RecursionFlagsBoth, false)
				if result != TernaryFalse {
					result &= r.isRelatedTo(source.AsConditionalType().extendsType, target.AsConditionalType().extendsType, RecursionFlagsBoth, false)
					if result != TernaryFalse {
						result &= r.isRelatedTo(r.c.getTrueTypeFromConditionalType(source), r.c.getTrueTypeFromConditionalType(target), RecursionFlagsBoth, false)
						if result != TernaryFalse {
							result &= r.isRelatedTo(r.c.getFalseTypeFromConditionalType(source), r.c.getFalseTypeFromConditionalType(target), RecursionFlagsBoth, false)
							if result != TernaryFalse {
								return result
							}
						}
		...
		case source.flags&TypeFlagsSubstitution != 0:
			result = r.isRelatedTo(source.AsSubstitutionType().baseType, target.AsSubstitutionType().baseType, RecursionFlagsBoth, false)
			if result != TernaryFalse {
				result &= r.isRelatedTo(source.AsSubstitutionType().constraint, target.AsSubstitutionType().constraint, RecursionFlagsBoth, false)
				if result != TernaryFalse {
					return result
				}
			}
		case source.flags&TypeFlagsTemplateLiteral != 0:
			if slices.Equal(source.AsTemplateLiteralType().texts, target.AsTemplateLiteralType().texts) {
				...
				for i, sourceType := range source.AsTemplateLiteralType().types {
					...
					result &= r.isRelatedTo(sourceType, targetType, RecursionFlagsBoth, false)
					if result == TernaryFalse {
						return result
					}
				}
		...
		case source.flags&TypeFlagsStringMapping != 0:
			if source.AsStringMappingType().Symbol() == target.AsStringMappingType().Symbol() {
				return r.isRelatedTo(source.AsStringMappingType().target, target.AsStringMappingType().target, RecursionFlagsBoth, false)
			}
		}
	...
	case source.flags&TypeFlagsUnionOrIntersection != 0 || target.flags&TypeFlagsUnionOrIntersection != 0:
		...
		if !(source.flags&TypeFlagsInstantiable != 0 || source.flags&TypeFlagsObject != 0 && target.flags&TypeFlagsUnion != 0 || source.flags&TypeFlagsIntersection != 0 && target.flags&(TypeFlagsObject|TypeFlagsUnion|TypeFlagsInstantiable) != 0) {
			return TernaryFalse
		}
	}
	if source.flags&(TypeFlagsObject|TypeFlagsConditional) != 0 && source.alias != nil && len(source.alias.typeArguments) != 0 && target.alias != nil && source.alias.symbol == target.alias.symbol && !(r.c.isMarkerType(source) || r.c.isMarkerType(target)) {
	...
	if isSingleElementGenericTupleType(source) && !source.TargetTupleType().readonly {
		result = r.isRelatedTo(r.c.getTypeArguments(source)[0], target, RecursionFlagsSource, false)
		if result != TernaryFalse {
			return result
		}
	}
	if isSingleElementGenericTupleType(target) && (target.TargetTupleType().readonly || r.c.isMutableArrayOrTuple(r.c.getBaseConstraintOrType(source))) {
		result = r.isRelatedTo(source, r.c.getTypeArguments(target)[0], RecursionFlagsTarget, false)
		if result != TernaryFalse {
			return result
		}
	}
	...
	case target.flags&TypeFlagsTypeParameter != 0:
		...
		if r.relation == r.c.comparableRelation && source.flags&TypeFlagsTypeParameter != 0 {
			if constraint := r.c.getConstraintOfTypeParameter(source); constraint != nil && someType(constraint, func(c *Type) bool {
				return c.flags&TypeFlagsTypeParameter != 0
			}) {
				return r.isRelatedTo(constraint, target, RecursionFlagsSource, false)
			}
			return TernaryFalse
		}
	case target.flags&TypeFlagsIndexedAccess != 0:
		...
		if r.relation == r.c.assignableRelation || r.relation == r.c.comparableRelation {
			...
			if !r.c.isGenericObjectType(baseObjectType) && !r.c.isGenericIndexType(baseIndexType) {
				...
				constraint := r.c.getIndexedAccessTypeOrUndefined(baseObjectType, baseIndexType, accessFlags, ast.Handle{}, nil)
				if constraint != nil {
					if reportErrors && originalErrorChain != nil {
						r.restoreErrorState(saveErrorState)
					}
					result = r.isRelatedToEx(source, constraint, RecursionFlagsTarget, reportErrors, nil, intersectionState)
					if result != TernaryFalse {
						return result
					}
	...
	case target.flags&TypeFlagsIndex != 0:
		...
		if source.flags&TypeFlagsIndex != 0 {
			result = r.isRelatedTo(targetType, source.AsIndexType().target, RecursionFlagsBoth, false)
			if result != TernaryFalse {
				return result
			}
		}
	...
	case target.flags&TypeFlagsConditional != 0:
		...
		if c.root.inferTypeParameters == nil && !r.c.isDistributionDependent(c.root) && !(source.flags&TypeFlagsConditional != 0 && source.AsConditionalType().root == c.root) {
			...
			if skipTrue {
				...
				result = r.isRelatedToEx(source, r.c.getTrueTypeFromConditionalType(target), RecursionFlagsTarget, false, nil, intersectionState)
			}
			if result != TernaryFalse {
				if skipFalse {
					result &= TernaryTrue
				} else {
					result &= r.isRelatedToEx(source, r.c.getFalseTypeFromConditionalType(target), RecursionFlagsTarget, false, nil, intersectionState)
				}
				if result != TernaryFalse {
					return result
				}
	...
	case source.flags&TypeFlagsTypeVariable != 0:
		if source.flags&TypeFlagsIndexedAccess == 0 || target.flags&TypeFlagsIndexedAccess == 0 {
			...
			constraintWithThis := r.c.getTypeWithThisArgument(constraint, source, false)
			result = r.isRelatedToEx(constraintWithThis, target, RecursionFlagsSource, reportErrors && constraint != r.c.unknownType && target.flags&source.flags&TypeFlagsTypeParameter == 0, nil, intersectionState)
			if result != TernaryFalse {
				return result
			}
	...
	case source.flags&TypeFlagsConditional != 0:
		...
		if target.flags&TypeFlagsConditional != 0 {
			...
			if len(sourceParams) != 0 {
				ctx := r.c.newInferenceContext(sourceParams, nil, InferenceFlagsNone, r.isRelatedToWorker)
	...
	default:
		...
		if source.flags&(TypeFlagsObject|TypeFlagsIntersection) != 0 && target.flags&TypeFlagsObject != 0 {
			...
			result = r.propertiesRelatedTo(source, target, reportStructuralErrors, collections.Set[string]{}, false, intersectionState)
			if result != TernaryFalse {
	...
	for i := range length {
		...
		if variance != VarianceFlagsIndependent {
			...
			if varianceFlags&VarianceFlagsUnmeasurable != 0 {
				if r.relation == r.c.identityRelation {
					related = r.isRelatedTo(s, t, RecursionFlagsBoth, false)
				} else {
				...
				if variance == VarianceFlagsCovariant {
					related = r.isRelatedToEx(s, t, RecursionFlagsBoth, reportErrors, nil, intersectionState)
				} else if variance == VarianceFlagsContravariant {
					related = r.isRelatedToEx(t, s, RecursionFlagsBoth, reportErrors, nil, intersectionState)
				} else if variance == VarianceFlagsBivariant {
					related = r.isRelatedTo(t, s, RecursionFlagsBoth, false)
					if related == TernaryFalse {
						related = r.isRelatedToEx(s, t, RecursionFlagsBoth, reportErrors, nil, intersectionState)
					}
				} else {
...
func (r *Relater) mappedTypeRelatedTo(source *Type, target *Type, reportErrors bool) Ternary {
	modifiersRelated := r.relation == r.c.comparableRelation || r.relation == r.c.identityRelation && getMappedTypeModifiers(source) == getMappedTypeModifiers(target) || r.relation != r.c.identityRelation && r.c.getCombinedMappedTypeOptionality(source) <= r.c.getCombinedMappedTypeOptionality(target)
	if modifiersRelated {
...
func (r *Relater) typeRelatedToDiscriminatedType(source *Type, target *Type) Ternary {
	...
	outer:
		for _, t := range target.Types() {
			for i := range sourcePropertiesFiltered {
				...
				related := r.propertyRelatedTo(source, target, sourceProperty, targetProperty, func(*ast.Symbol) *Type {
					return combination[i]
				}, false, IntersectionStateNone, r.c.strictNullChecks || r.relation == r.c.comparableRelation)
				if related == TernaryFalse {
					continue outer
				}
			}
	...
	for _, t := range matchingTypes {
		result &= r.propertiesRelatedTo(source, t, false, excludedProperties, false, IntersectionStateNone)
		if result != TernaryFalse {
			result &= r.signaturesRelatedTo(source, t, SignatureKindCall, false, IntersectionStateNone)
			if result != TernaryFalse {
				result &= r.signaturesRelatedTo(source, t, SignatureKindConstruct, false, IntersectionStateNone)
				if result != TernaryFalse && !(isTupleType(source) && isTupleType(t)) {
					result &= r.indexSignaturesRelatedTo(source, t, false, false, IntersectionStateNone)
				}
			}
		}
		if result == TernaryFalse {
...
func (r *Relater) propertiesRelatedTo(source *Type, target *Type, reportErrors bool, excludedProperties collections.Set[string], optionalsOnly bool, intersectionState IntersectionState) Ternary {
	...
	if isTupleType(target) {
		if r.c.isArrayOrTupleType(source) {
			...
			for sourcePosition := range sourceArity {
				...
				related := r.isRelatedToEx(sourceType, targetCheckType, RecursionFlagsBoth, reportErrors, nil, intersectionState)
				if related == TernaryFalse {
	...
	unmatchedProperty := r.c.getUnmatchedProperty(source, target, requireOptionalProperties, false)
...
func (r *Relater) isPropertySymbolTypeRelated(sourceProp *ast.Symbol, targetProp *ast.Symbol, getTypeOfSourceProperty func(sym *ast.Symbol) *Type, reportErrors bool, intersectionState IntersectionState) Ternary {
	...
	effectiveTarget := r.c.addOptionalityEx(r.c.getNonMissingTypeOfSymbol(targetProp), false, targetIsOptional)
	...
	return r.isRelatedToEx(effectiveSource, effectiveTarget, RecursionFlagsBoth, reportErrors, nil, intersectionState)
}
func (r *Relater) reportUnmatchedProperty(source *Type, target *Type, unmatchedProperty *ast.Symbol, requireOptionalProperties bool) {
	if !unmatchedProperty.ValueDeclaration.IsNil() && !unmatchedProperty.ValueDeclaration.Name().IsNil() && ast.IsPrivateIdentifier(unmatchedProperty.ValueDeclaration.Name()) && source.symbol != nil && source.symbol.Flags&ast.SymbolFlagsClass != 0 {
	...
	props := r.c.getUnmatchedProperties(source, target, requireOptionalProperties, false)
	if len(props) == 1 {
	...
	} else if r.tryElaborateArrayLikeErrors(source, target, false) {
		sourceType, targetType := r.c.getTypeNamesForErrorDisplay(source, target)
		if len(props) > 5 {
...
func (r *Relater) tryElaborateErrorsForPrimitivesAndObjects(source *Type, target *Type) {
	if (source == r.c.globalStringType && target == r.c.stringType) || (source == r.c.globalNumberType && target == r.c.numberType) || (source == r.c.globalBooleanType && target == r.c.booleanType) || (source == r.c.getGlobalESSymbolType() && target == r.c.esSymbolType) {
...
func (r *Relater) signaturesRelatedTo(source *Type, target *Type, kind SignatureKind, reportErrors bool, intersectionState IntersectionState) Ternary {
	...
	result := TernaryTrue
	switch {
	case source.objectFlags&ObjectFlagsInstantiated != 0 && target.objectFlags&ObjectFlagsInstantiated != 0 && source.symbol == target.symbol || source.objectFlags&ObjectFlagsReference != 0 && target.objectFlags&ObjectFlagsReference != 0 && source.Target() == target.Target():
		for i := range targetSignatures {
			related := r.signatureRelatedTo(sourceSignatures[i], targetSignatures[i], true, reportErrors, intersectionState)
			if related == TernaryFalse {
				return TernaryFalse
			}
			result &= related
	...
	outer:
		for _, t := range targetSignatures {
			...
			for _, s := range sourceSignatures {
				related := r.signatureRelatedTo(s, t, true, shouldElaborateErrors, intersectionState)
				if related != TernaryFalse {
					result &= related
					r.restoreErrorState(saveErrorState)
					continue outer
...
func (r *Relater) signatureRelatedTo(source *Signature, target *Signature, erase bool, reportErrors bool, intersectionState IntersectionState) Ternary {
	...
	isRelatedToWorker := func(source *Type, target *Type, reportErrors bool) Ternary {
		return r.isRelatedToEx(source, target, RecursionFlagsBoth, reportErrors, nil, intersectionState)
	}
...
func (r *Relater) signaturesIdenticalTo(source *Type, target *Type, kind SignatureKind) Ternary {
	...
	for i := range sourceSignatures {
		related := r.c.compareSignaturesIdentical(sourceSignatures[i], targetSignatures[i], false, false, false, r.isRelatedToSimple)
		if related == 0 {
			return TernaryFalse
		}
		result &= related
...
func (r *Relater) indexSignaturesRelatedTo(source *Type, target *Type, sourceIsPrimitive bool, reportErrors bool, intersectionState IntersectionState) Ternary {
	...
	targetHasStringIndex := core.Some(indexInfos, func(info *IndexInfo) bool {
		return info.keyType == r.c.stringType
	})
	result := TernaryTrue
...
func (c *Checker) isObjectTypeWithInferableIndex(t *Type) bool {
	...
	return t.symbol != nil && t.symbol.Flags&(ast.SymbolFlagsObjectLiteral|ast.SymbolFlagsTypeLiteral|ast.SymbolFlagsEnum|ast.SymbolFlagsValueModule) != 0 && t.symbol.Flags&ast.SymbolFlagsClass == 0 && !c.typeHasCallOrConstructSignatures(t) || t.objectFlags&(ObjectFlagsJSLiteral|ObjectFlagsObjectRestType) != 0 || t.objectFlags&ObjectFlagsReverseMapped != 0 && c.isObjectTypeWithInferableIndex(t.AsReverseMappedType().source)
}
func (r *Relater) membersRelatedToIndexInfo(source *Type, targetInfo *IndexInfo, reportErrors bool, intersectionState IntersectionState) Ternary {
	...
	for _, prop := range props {
		...
		if r.c.isApplicableIndexType(r.c.getLiteralTypeFromProperty(prop, TypeFlagsStringOrNumberLiteralOrUnique, false), keyType) {
			...
			related := r.isRelatedToEx(t, targetInfo.valueType, RecursionFlagsBoth, reportErrors, nil, intersectionState)
			if related == TernaryFalse {
				if reportErrors {
...
func (r *Relater) indexInfoRelatedTo(sourceInfo *IndexInfo, targetInfo *IndexInfo, reportErrors bool, intersectionState IntersectionState) Ternary {
	related := r.isRelatedToEx(sourceInfo.valueType, targetInfo.valueType, RecursionFlagsBoth, reportErrors, nil, intersectionState)
	if related == TernaryFalse && reportErrors {
...
func (r *Relater) reportErrorResults(originalSource *Type, originalTarget *Type, source *Type, target *Type, headMessage *diagnostics.Message) {
	...
	if source.flags&TypeFlagsObject != 0 && target.flags&TypeFlagsObject != 0 {
		r.tryElaborateArrayLikeErrors(source, target, true)
	}
	switch {
	...
	case originalTarget.flags&TypeFlagsIntersection != 0 && originalTarget.objectFlags&ObjectFlagsIsNeverIntersection != 0:
		...
		if prop != nil {
			r.reportError(message, r.c.typeToStringEx(originalTarget, ast.Handle{}, TypeFormatFlagsNoTypeReduction, nil), r.c.symbolToString(prop))
		}
	}
...
func (r *Relater) reportRelationError(message *diagnostics.Message, source *Type, target *Type) {
	...
	case diagnostics.Object_literal_may_only_specify_known_properties_and_0_does_not_exist_in_type_1, diagnostics.Object_literal_may_only_specify_known_properties_but_0_does_not_exist_in_type_1_Did_you_mean_to_write_2:
		return
	case diagnostics.Excessive_complexity_comparing_types_0_and_1, diagnostics.The_type_0_is_readonly_and_cannot_be_assigned_to_the_mutable_type_1:
	...
	case diagnostics.Type_0_is_missing_the_following_properties_from_type_1_Colon_2_and_3_more, diagnostics.Type_0_is_missing_the_following_properties_from_type_1_Colon_2:
...
func (r *Relater) reportError(message *diagnostics.Message, args ...any) {
	if message == diagnostics.Types_of_property_0_are_incompatible {
		...
		switch r.getChainMessage(1) {
		case diagnostics.Types_of_property_0_are_incompatible, diagnostics.The_types_of_0_are_incompatible_between_these_types, diagnostics.The_types_returned_by_0_are_incompatible_between_these_types:
...
func isConversionOrInterfaceImplementationMessage(message *diagnostics.Message) bool {
	return message == diagnostics.Class_0_incorrectly_implements_interface_1 || message == diagnostics.Class_0_incorrectly_implements_class_1_Did_you_mean_to_extend_1_and_inherit_its_members_as_a_subclass || message == diagnostics.Conversion_of_type_0_to_type_1_may_be_a_mistake_because_neither_type_sufficiently_overlaps_with_the_other_If_this_was_intentional_convert_the_expression_to_unknown_first || message == diagnostics.Its_instance_type_0_is_not_a_valid_JSX_element || message == diagnostics.Its_return_type_0_is_not_a_valid_JSX_element || message == diagnostics.Its_element_type_0_is_not_a_valid_JSX_element
}
...
func (c *Checker) isDistributionDependent(root *ConditionalRoot) bool {
	return root.isDistributive && (c.isTypeParameterPossiblyReferenced(root.checkType, root.node.ConditionalTypeNodeTrueType()) || c.isTypeParameterPossiblyReferenced(root.checkType, root.node.ConditionalTypeNodeFalseType()))
}
