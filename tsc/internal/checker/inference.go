...
type InferenceState struct {
	...
	// matching types.
	/*sort*/ // Next, infer between closely matching source and target constituents and remove
	// the matching types. Types closely match when they are instantiations of the same
	// object type or instantiations of the same type alias.
	/*sort*/ // All source constituents have been matched and there is nothing further to infer from.
	...
	/*sort*/ // Skip inference if the source is "blocked", which is used by the language service to
	...
	/*writing*/ /*writing*/ // Generally simplifications of instantiable indexes are avoided to keep relationship checking correct, however if our target is an access, we can consider
	// that key of that access to be "instantiated", since we're looking to find the infernce goal in any way we can.
	/*writing*/ /*writing*/ // If source and target are references to the same generic type, infer from type arguments
	...
	/*roundTripOnly*/ // If the type contains `bigint` or a bigint literal and the string isn't a valid bigint, exclude bigints
	/*roundTripOnly*/ // The source and target types are generic types { [P in S]: X } and { [P in T]: Y }, so we infer
	...
	// if T is constrained by a fixed-size tuple we might be able to use its arity to infer T
	/*readonly*/ // Middle of target is exactly one variadic element. Infer the slice between the fixed parts in the source.
	...
	/*readonly*/ // Inferences across mapped type index signatures are pretty much the same a inferences to homomorphic variables
	...
	/*symbol*/ // We consider a type to be partially inferable if it isn't marked non-inferable or if it is
	...
	/*requireOptionalProperties*/ /*matchDiscriminantProperties*/ /*requireOptionalProperties*/ /*matchDiscriminantProperties*/ // Create an object with properties named in the string literal type. Every property has type `any`.
	/*isReadonly*/ /*fixing*/ /*fixing*/ // We collect intra-expression inference sites within object and array literals to handle cases where
...
func (c *Checker) putInferenceState(n *InferenceState) {
	clear(n.visited)
	*n = InferenceState{inferences: n.inferences[:0], visited: n.visited, sourceStack: n.sourceStack[:0], targetStack: n.targetStack[:0], next: c.freeinferenceState}
...
func (c *Checker) inferFromTypes(n *InferenceState, source *Type, target *Type) {
	...
	if target.flags&TypeFlagsUnion != 0 {
		...
		tempSources, tempTargets := c.inferFromMatchingTypes(n, sourceTypes, target.Types(), (*Checker).isTypeOrBaseIdenticalTo, false)
		sources, targets := c.inferFromMatchingTypes(n, tempSources, tempTargets, (*Checker).isTypeCloselyMatchedBy, true)
		if len(targets) == 0 {
			return
		}
		...
		if source.flags&TypeFlagsUnion == 0 {
			...
			sources, targets := c.inferFromMatchingTypes(n, sourceTypes, target.Types(), (*Checker).isTypeIdenticalTo, false)
	...
	if target.flags&TypeFlagsTypeVariable != 0 {
		...
		simplified := c.getSimplifiedType(target, false)
		if simplified != target {
			c.inferFromTypes(n, source, simplified)
		} else if target.flags&TypeFlagsIndexedAccess != 0 {
			indexType := c.getSimplifiedType(target.AsIndexedAccessType().indexType, false)
			if indexType.flags&TypeFlagsInstantiable != 0 {
				simplified := c.distributeIndexOverObjectType(c.getSimplifiedType(target.AsIndexedAccessType().objectType, false), indexType, false)
...
func (c *Checker) inferFromMatchingTypes(n *InferenceState, sources []*Type, targets []*Type, matches func(c *Checker, s *Type, t *Type) bool, sort bool) ([]*Type, []*Type) {
	...
	if len(matchedSources) != 0 {
		sources = core.Filter(sources, func(t *Type) bool {
			return !slices.Contains(matchedSources, t)
		})
	}
	if len(matchedTargets) != 0 {
		targets = core.Filter(targets, func(t *Type) bool {
			return !slices.Contains(matchedTargets, t)
		})
	}
...
func getSingleTypeVariableFromIntersectionTypes(n *InferenceState, types []*Type) *Type {
	...
	for _, t := range types {
		...
		v := core.Find(t.Types(), func(t *Type) bool {
			return getInferenceInfoForType(n, t) != nil
		})
...
func (c *Checker) inferToTemplateLiteralType(n *InferenceState, source *Type, target *TemplateLiteralType) {
	...
	types := target.types
	if len(matches) != 0 || core.Every(target.texts, func(s string) bool {
		return s == ""
	}) {
						...
						if info := getInferenceInfoForType(n, elementTypes[startLength+1]); info != nil {
							...
							if constraint != nil && isTupleType(constraint) && constraint.TargetTupleType().combinedFlags&ElementFlagsVariable == 0 {
								...
								if startIndex >= startLength {
									trailingSlice := c.createTupleTypeEx(c.getTypeArguments(source)[startIndex:endIndex], source.TargetTupleType().elementInfos[startIndex:endIndex], false)
...
func (c *Checker) inferFromSignature(n *InferenceState, source *Signature, target *Signature) {
	if source.flags&SignatureFlagsIsNonInferrable == 0 {
		...
		c.applyToParameterTypes(source, target, func(s, t *Type) {
			c.inferFromContravariantTypesIfStrictFunctionTypes(n, s, t)
		})
		n.bivariant = saveBivariant
	}
	c.applyToReturnTypes(source, target, func(s, t *Type) {
		c.inferFromTypes(n, s, t)
	})
}
func (c *Checker) applyToParameterTypes(source *Signature, target *Signature, callback func(s *Type, t *Type)) {
	...
	if targetRestType != nil {
		callback(c.getRestTypeAtPosition(source, paramCount, c.isConstTypeVariable(targetRestType, 0) && !someType(targetRestType, c.isMutableArrayLikeType)), targetRestType)
	}
}
...
func (c *Checker) createReverseMappedType(source *Type, target *Type, constraint *Type) *Type {
	...
	reversed := c.newObjectType(ObjectFlagsReverseMapped|ObjectFlagsAnonymous, nil)
...
func (c *Checker) resolveReverseMappedTypeMembers(t *Type) {
	...
	if indexInfo != nil {
		indexInfos = []*IndexInfo{c.newIndexInfo(c.stringType, core.OrElse(c.inferReverseMappedType(indexInfo.valueType, r.mappedType, r.constraintType), c.unknownType), readonlyMask && indexInfo.isReadonly, ast.Handle{}, nil)}
	}
...
func (c *Checker) getLimitedConstraint(t *Type) *Type {
	...
	limitedConstraint := c.getIntersectionType(core.Filter(origin.Types(), func(t *Type) bool {
		return t != constraintType
	}))
...
func (c *Checker) typesDefinitelyUnrelated(source *Type, target *Type) bool {
	...
	return c.getUnmatchedProperty(source, target, false, true) != nil && c.getUnmatchedProperty(target, source, false, false) != nil
}
func tupleTypesDefinitelyUnrelated(source *Type, target *Type) bool {
	s := source.TargetTupleType()
	t := target.TargetTupleType()
	return t.combinedFlags&ElementFlagsVariadic == 0 && t.minLength > s.minLength || t.combinedFlags&ElementFlagsVariable == 0 && (s.combinedFlags&ElementFlagsVariable != 0 || t.fixedLength < s.fixedLength)
}
...
func (c *Checker) isTypeCloselyMatchedBy(s *Type, t *Type) bool {
	return s.flags&TypeFlagsObject != 0 && t.flags&TypeFlagsObject != 0 && s.symbol != nil && s.symbol == t.symbol || s.alias != nil && t.alias != nil && len(s.alias.typeArguments) != 0 && s.alias.symbol == t.alias.symbol
}
...
func (c *Checker) createEmptyObjectTypeFromStringLiteral(t *Type) *Type {
	...
	if t.flags&TypeFlagsString != 0 {
		indexInfos = []*IndexInfo{c.newIndexInfo(c.stringType, c.emptyObjectType, false, ast.Handle{}, nil)}
	}
...
func (c *Checker) newInferenceContextWorker(inferences []*InferenceInfo, signature *Signature, flags InferenceFlags, compareTypes TypeComparer) *InferenceContext {
	...
	n.mapper = c.newInferenceTypeMapper(n, true)
	n.nonFixingMapper = c.newInferenceTypeMapper(n, false)
	return n
}
func (c *Checker) addIntraExpressionInferenceSite(n *InferenceContext, node ast.Handle, t *Type) {
	...
	if inference.inferredType == nil {
		...
		if n.signature != nil {
			...
			if inferredCovariantType != nil || inferredContravariantType != nil {
				preferCovariantType := inferredCovariantType != nil && (inferredContravariantType == nil || inferredCovariantType.flags&(TypeFlagsNever|TypeFlagsAny) == 0 && core.Some(inference.contraCandidates, func(t *Type) bool {
...
func (c *Checker) unionObjectAndArrayLiteralCandidates(candidates []*Type) []*Type {
	if len(candidates) > 1 {
		...
		if len(objectLiterals) != 0 {
			...
			nonLiteralTypes := core.Filter(candidates, func(t *Type) bool {
...
func (c *Checker) isTypeParameterAtTopLevel(t *Type, tp *Type, depth int) bool {
	return t == tp || t.flags&TypeFlagsUnionOrIntersection != 0 && core.Some(t.Types(), func(t *Type) bool {
		return c.isTypeParameterAtTopLevel(t, tp, depth)
	}) || depth < 3 && t.flags&TypeFlagsConditional != 0 && (c.isTypeParameterAtTopLevel(c.getTrueTypeFromConditionalType(t), tp, depth+1) || c.isTypeParameterAtTopLevel(c.getFalseTypeFromConditionalType(t), tp, depth+1))
}
...
func (c *Checker) getSingleCommonSupertype(types []*Type) *Type {
	...
	if core.Every(types, func(t *Type) bool {
		return t == candidate || c.isTypeStrictSubtypeOf(t, candidate)
	}) {
		return candidate
	}
...
func (c *Checker) isSkipDirectInferenceNode(node ast.Handle) bool {
...
func cloneInferenceInfo(info *InferenceInfo) *InferenceInfo {
	return &InferenceInfo{typeParameter: info.typeParameter, candidates: slices.Clone(info.candidates), contraCandidates: slices.Clone(info.contraCandidates), inferredType: info.inferredType, priority: info.priority, topLevel: info.topLevel, isFixed: info.isFixed, impliedArity: info.impliedArity}
}
...
func hasTypeParameterDefault(tp *Type) bool {
	if tp.symbol != nil {
		for _, d := range ast.DeclarationNodes(tp.symbol) {
			if ast.IsTypeParameterDeclaration(d) && !d.TypeParameterDeclarationDefaultType().IsNil() {
				return true
			}
		}
	}
