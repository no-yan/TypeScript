...
func (c *Checker) GetStringType( /* withAugmentations */ /*ignoreErrors*/ /*dontResolveAlias*/ // Checks if a property can be accessed in a location.
...
func (c *Checker) GetTypeOnlyAliasDeclaration(symbol *ast.Symbol) ast.Handle {
	return c.getTypeOnlyAliasDeclaration(symbol)
}
func (c *Checker) ResolveExternalModuleName(moduleSpecifier ast.Handle) *ast.Symbol {
...
func (c *Checker) GetTypeFromTypeNode(node ast.Handle) *Type {
...
func (c *Checker) IsPropertyAccessible(node ast.Handle, isSuper bool, isWrite bool, containingType *Type, property *ast.Symbol) bool {
...
func (c *Checker) GetEffectiveDeclarationFlags(n ast.Handle, flagsToCheck ast.ModifierFlags) ast.ModifierFlags {
...
func (c *Checker) GetContextualTypeForObjectLiteralElement(element ast.Handle, contextFlags ContextFlags) *Type {
...
func (c *Checker) GetResolvedSignature(node ast.Handle) *Signature {
...
func (c *Checker) GetContextualTypeForArgumentAtIndex(node ast.Handle, argIndex int) *Type {
	return c.getContextualTypeForArgumentAtIndex(node, argIndex)
}
func (c *Checker) GetIndexSignaturesAtLocation(node ast.Handle) []ast.Handle {
	return c.getIndexSignaturesAtLocation(node)
}
func (c *Checker) GetResolvedSymbol(node ast.Handle) *ast.Symbol {
...
func (c *Checker) GetJsxFragmentFactory(location ast.Handle) string {
...
func (c *Checker) ResolveName(name string, location ast.Handle, meaning ast.SymbolFlags, excludeGlobals bool) *ast.Symbol {
...
func (c *Checker) GetFullyQualifiedName(symbol *ast.Symbol) string {
	return c.getFullyQualifiedName(symbol, ast.Handle{})
}
func (c *Checker) GetBaseConstructorTypeOfClass(t *Type) *Type {
	return c.getBaseConstructorTypeOfClass(t)
}
func (c *Checker) GetMemberOverrideModifierStatus(node ast.Handle, member ast.Handle, memberSymbol *ast.Symbol) MemberOverrideStatus {
...
func (c *Checker) IsContextSensitive(node ast.Handle) bool {
...
func (c *Checker) RequiresAddingImplicitUndefined(node ast.Handle) bool {
	enclosingDeclaration := ast.FindAncestor(node, ast.IsDeclaration)
	if enclosingDeclaration.IsNil() {
		enclosingDeclaration = ast.GetSourceFileOfNode(node).ParseRoot()
	}
	symbol := node.Symbol()
	if symbol == nil {
		return false
