...
| `flows []*FlowNode` | dense pointer column sized at `PrepareBindTables` | fill is about 50%, so indexing does not shrink it; see Open work |
...
8. **Flow is a second pointer graph.** `FlowNode{Flags, Node Handle, Data *Node, Antecedent *FlowNode, Antecedents *FlowList}` is 48 bytes with four pointer words. Putting `*FlowNode` in `nodeHeader` would make `[]nodeHeader` scannable. Flow pointers live in the `flows` column and side maps; the header stays noscan.
...
5. **`flows` column and allocation hints.** Record per-Store column fill after bind and check; the `len(sourceText)/10` hint over-reserves on large files.
