...
cd $TSC && git diff --stat HEAD -- internal/binder internal/parser internal/scanner internal/ast ':!internal/ast/store' ':!internal/ast/docs'   # 空
...
4. **bind の等価**: driver が比べる項目が実装指示書 §6.1 の全部であることを driver のコードで確認する (項目が抜けていれば mismatch 0 は意味が無い)。private 名の正規化 (`#<id>@` → `#@`) 以外の緩めが無いこと。
...
| `bind` | 自分の header を読む `MOVH` / `MOVWU` はあるが、`nodes` の base + index の計算 (`node()` の形) は子を辿る箇所以外に無い |
...
Bind が 1.10 を超えたら、7c 設計 §2.2 の順に理由を探す: (1) `node()` が辺 1 本に 1 回になっているか (§3)、(2) `Ref()` の回数 (overlay の計数で 0.7 回/node 前後か)、(3) `IsIdentifierName` の Parent + `Name()` (0.41 × 20 inst)、(4) 予約 slot の setter が表引きになっていないか、(5) 1 word 通貨 (`*NodeHeader`) の試作。順に切り分けて、どれで説明できるかを報告に書く。
...
cd $TSC && go test -c -o $OUT/storebinder.test ./internal/storebinder/
cd $TSC/internal/storebinder && GODEBUG=gctrace=1 $OUT/storebinder.test -test.run '^$' -test.bench 'StoreBindRetainedV1/store' -test.benchtime 1x -test.count 1 2> $OUT/gctrace-store.txt
cd $TSC/internal/storebinder && GODEBUG=gctrace=1 $OUT/storebinder.test -test.run '^$' -test.bench 'StoreBindRetainedV1/pointer' -test.benchtime 1x -test.count 1 2> $OUT/gctrace-pointer.txt
```

gctrace はテストバイナリを直接実行する (`go test` 経由だと go コマンド自身の GC が混じる。7b 報告 §10)。retained は GC 2 回の後の値 (7b 報告 §7.2)。

見積り (checker.ts 換算、B/node): Store 35.1 (7b) + 8 (header) + 0.4 (slot) + 2.9 (flow slab) + 5.9 (symbol) + 0.5 (`symbols` 表) + table と declarations ≈ 55。Pointer 94 + flow 5.7 + symbol 5.9 + 同じ table ≈ 110。fixtures 166 file は dom 系が多いので symbol の比重が上がる。

| 指標 | 線 | 状態 |
| --- | --- | --- |
| retained B/node、store / pointer (parse + bind) | ≤ 0.60 (見積り 0.50) | **提案。ユーザーが決める** |
| 保持中の `runtime.GC()` 1 回の ms、store / pointer | ≤ 0.15 (7b は 0.03。`Bound` の symbol と map が scan 対象に加わる) | **提案。ユーザーが決める** |
