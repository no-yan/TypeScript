...
| 3 | `getDeclarationName` | ~~`IsJsxNamespacedName(name)` の枝を分け `jsxNamespacedNameText(name)`~~ 2026-09-23 に解消: `Node.Text()` を `store.go` の手書きにして MetaProperty / JsxNamespacedName の case を足し、Pointer と同じ `IsPropertyNameLiteral(name) \|\| IsJsxNamespacedName(name)` → `name.Text()` に戻した。`IsImportCall` も `e.Text()` に戻した | kind 3 | kind 3 |
...
`storebinder/utilities.go`: getScannerForSourceFile、getRangeOfTokenAtPosition、getErrorRangeForArrowFunction、getECMAEndLinePosition、isLineBreak、getErrorRangeForNode、getSourceTextOfNodeFromSourceFile、getTextOfNodeFromSourceText、declarationNameToString (`jsxNamespacedNameText` は 2026-09-23 に削除)。
...
4. **`store.Node.Text()` の欠落**: Pointer の `Text()` にある JsxNamespacedName (`ns:name`) と MetaProperty (`Name().Text()`) の case が生成 `Text()` に無い。binder 側で `jsxNamespacedNameText` を分け、`IsImportCall` は `AsMetaProperty().Name().Text()` を読む。生成側に足すなら役割 `Text()` の分岐が増える (設計文書 §2.3 の inline 予算の話)。 **決定と実装 (2026-09-23)**: `Node.Text()` を手書きにする (generator は `Text` の関数を出さず、`textSlot` の表だけを出す)。Pointer の `(*ast.Node).Text` と同じく kind の switch (Identifier / PrivateIdentifier、MetaProperty、JsxNamespacedName) の後に表引き。JsxNamespacedName は JSX の構文解析からしか作られないので、JSX でない file には影響しない。`TestComposedText` (storeparser) が 2 kind の `Text()` を Pointer と突き合わせる。
