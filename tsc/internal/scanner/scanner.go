...
var textToKeyword = map[ // Current position in text (and ending position of current token)
...
/*jsxAttributeString*/ /*shouldEmitInvalidEscapeError*/ // Single-line comment
// Multi-line comment
...
/*allowMultilineJsxText*/ // First non-whitespace character on this line.
...
/*jsxAttributeString*/ // If this scans anything other than `{`, it's a parse error.
/** In addition to the usual JSDoc ast.Kinds, can also return ast.KindJSDocCommentTextToken */ // @ doesn't start a new tag inside ``, and elsewhere, only after whitespace and before identifier
...
/*includeJSDoc*/ // The arrow function spans multiple lines, make the error span be the first line, inclusive.
...
func (s *Scanner) scanJSDocCommentForTags(commentText string) {
	for {
		...
		if s.tokenFlags&(ast.TokenFlagsPrecedingJSDocWithDeprecated|ast.TokenFlagsPrecedingJSDocWithSeeOrLink) == (ast.TokenFlagsPrecedingJSDocWithDeprecated | ast.TokenFlagsPrecedingJSDocWithSeeOrLink) {
			return
		}
	}
}
...
func (s *Scanner) Scan() ast.Kind {
	...
	for {
		...
		case '"', '\'':
			s.tokenValue = s.scanString(false)
			s.token = ast.KindStringLiteral
		case '`':
			s.token = s.scanTemplateAndSetTokenValue(false)
		case '%':
			if s.charAt(1) == '=' {
				s.pos += 2
		...
		case '*':
			...
			if next == '=' {
				...
				s.pos++
				if s.skipJSDocLeadingAsterisks != 0 && (s.tokenFlags&ast.TokenFlagsPrecedingJSDocLeadingAsterisks) == 0 && (s.tokenFlags&ast.TokenFlagsPrecedingLineBreak) != 0 {
...
func (s *Scanner) ReScanSlashToken(reportErrors ...bool) ast.Kind {
	...
	loop:
		for {
			...
			case ch == ']':
				inCharacterClass = false
			case !inCharacterClass && ch == '(' && p+1 < s.end && s.text[p+1] == '?' && p+2 < s.end && s.text[p+2] == '<' && (p+3 >= s.end || (s.text[p+3] != '=' && s.text[p+3] != '!')):
		...
		if s.tokenFlags&ast.TokenFlagsUnterminated != 0 {
			...
			if shouldReportErrors {
				...
				parser := &regExpParser{scanner: s, end: endOfRegExpBody, regExpFlags: regExpFlags, anyUnicodeMode: regExpFlags&regularExpressionFlagsAnyUnicodeMode != 0, unicodeSetsMode: regExpFlags&regularExpressionFlagsUnicodeSets != 0, annexB: true, namedCaptureGroups: namedCaptureGroups, groupSpecifiers: make(map[string]bool)}
				parser.run()
				s.end = saveEnd
				s.pos = p
...
func (s *Scanner) ScanJsxAttributeValue() ast.Kind {
	...
	switch s.char() {
	case '"', '\'':
		s.tokenValue = s.scanString(true)
		s.token = ast.KindStringLiteral
		return s.token
	default:
		return s.Scan()
...
func (s *Scanner) scanEscapeSequence(flags EscapeSequenceScanningFlags) string {
	...
	case 'u':
		...
		if codePoint < 0 {
			...
			} else if flags&EscapeSequenceScanningFlagsAnyUnicodeMode != 0 && s.char() == '\\' && s.charAt(1) == 'u' && s.charAt(2) != '{' {
				savedPos := s.pos
...
func IsIdentifierPartEx(ch rune, languageVariant core.LanguageVariant) bool {
	return isWordCharacter(ch) || ch == '$' || ch >= utf8.RuneSelf && stringutil.IsUnicodeIdentifierPart(ch) || languageVariant == core.LanguageVariantJSX && (ch == '-' || ch == ':')
}
...
func GetTokenPosOfNode(node ast.Handle, sourceFile *ast.SourceFile, includeJSDoc bool) int {
...
func findOriginatingJSDocSatisfiesTag(sourceFile *ast.SourceFile, node ast.Handle) ast.Handle {
	targetType := node.SatisfiesExpressionType()
...
func GetErrorRangeForNode(sourceFile *ast.SourceFile, node ast.Handle) core.TextRange {
	...
	case ast.KindSatisfiesExpression:
		...
		pos := SkipTrivia(sourceFile.Text(), node.SatisfiesExpressionExpression().End())
		return GetRangeOfTokenAtPosition(sourceFile, pos)
...
func iterateCommentRanges(text string, pos int, trailing bool) iter.Seq[ast.CommentRange] {
	...
	scan:
		for pos >= 0 && pos < len(text) {
			...
			case '/':
				...
				if nextChar == '/' || nextChar == '*' {
					...
					if collecting {
						if hasPendingCommentRange {
							if !yield(ast.CommentRange{TextRange: core.NewTextRange(pendingPos, pendingEnd), Kind: pendingKind, HasTrailingNewLine: pendingHasTrailingNewLine}) {
								return
							}
						}
						pendingPos = startPos
		...
		if hasPendingCommentRange {
			yield(ast.CommentRange{TextRange: core.NewTextRange(pendingPos, pendingEnd), Kind: pendingKind, HasTrailingNewLine: pendingHasTrailingNewLine})
		}
	}
}
