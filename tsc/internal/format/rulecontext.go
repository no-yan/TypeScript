...
func isBinaryOpContext(context *FormattingContext) bool {
	...
	case ast.KindVariableDeclaration:
		fallthrough
	case ast.KindParameter, ast.KindEnumMember, ast.KindPropertyDeclaration, ast.KindPropertySignature:
...
func nodeIsBlockContext(node ast.Handle) bool {
...
func isFunctionDeclContext(context *FormattingContext) bool {
	...
	case ast.KindGetAccessor, ast.KindSetAccessor:
		fallthrough
	case ast.KindCallSignature, ast.KindFunctionExpression, ast.KindConstructor, ast.KindArrowFunction:
		fallthrough
...
func isEndOfDecoratorContextOnSameLine(context *FormattingContext) bool {
	return context.TokensAreOnSameLine() && ast.HasDecorators(context.contextNode) && nodeIsInDecoratorContext(context.currentTokenParent) && !nodeIsInDecoratorContext(context.nextTokenParent)
}
func nodeIsInDecoratorContext(node ast.Handle) bool {
...
func isSemicolonDeletionContext(context *FormattingContext) bool {
	...
	if ast.IsTrivia(nextTokenKind) {
		var nextRealToken ast.Handle
		if context.nextTokenParent == context.currentTokenParent {
			nextRealToken = astnav.FindNextToken(context.nextTokenParent, context.SourceFile.ParseRoot(), context.SourceFile)
		} else {
	...
	if nextTokenKind == ast.KindSemicolonToken && context.currentTokenSpan.Kind == ast.KindSemicolonToken {
		return true
	}
...
func isNotPropertyAccessOnIntegerLiteral(context *FormattingContext) bool {
	return !ast.IsPropertyAccessExpression(context.contextNode) || !ast.IsNumericLiteral(context.contextNode.Expression()) || strings.Contains(context.contextNode.Expression().Text(), ".")
}
