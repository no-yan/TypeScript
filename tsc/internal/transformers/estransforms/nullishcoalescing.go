...
type nullishCoalescingTransformer struct{ transformers.Transformer }

func (ch *nullishCoalescingTransformer) visit(node ast.Handle) ast.Handle {
...
func (ch *nullishCoalescingTransformer) visitBinaryExpression(node ast.Handle) ast.Handle {
	...
	case ast.KindQuestionQuestionToken:
		...
		return ch.Factory().NewConditionalExpression(createNotNullCondition(ch.EmitContext(), left, right, false), ch.Factory().NewToken(ast.KindQuestionToken), right, ch.Factory().NewToken(ast.KindColonToken), ch.Visitor().VisitNode(node.Right()))
	default:
		return ch.Visitor().VisitEachChild(node)
	}
}
