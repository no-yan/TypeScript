...
func (w *displayPartsWriter) addRun(classification lsproto.ClassificationTypeName, text string) {
	...
	if w.vsCapability {
		w.runs = append(w.runs, &lsproto.VSClassifiedTextRun{ClassificationTypeName: string(classification), Text: text})
	}
	w.lastWritten = text
...
func (w displayPartsWriter) GetColumn() core.UTF16Offset {
	return 0
}
func (w displayPartsWriter) GetIndent() int {
	return 0
}
func (w displayPartsWriter) GetLine() int {
	return 0
}
func (w displayPartsWriter) GetTextPos() int {
	return w.builder.Len()
}
func (w displayPartsWriter) HasTrailingComment() bool {
	return false
}
...
func (w displayPartsWriter) IsAtStartOfLine() bool {
	return false
}
