...
type bucketBuildPreferences struct {
	fileExcludePatterns []string// bucketBuildPreferences holds user preferences that affect how a bucket is
	...
	/*enableDirectorySearch*/ // For packages whose main extraction yielded nothing, fall back to @types.
	// @types fallback packages always get directory search
	/*enableDirectorySearch*/ // --- Phase 3: Bucket building (parallel per bucket) ---
	...
	// Empty string for project buckets
	...
	// bucket directory path (used as extraction context)
	// true if realpath is within the workspace root
...
func bucketBuildPreferencesFromUserPreferences(prefs lsutil.UserPreferences) bucketBuildPreferences {
	return bucketBuildPreferences{fileExcludePatterns: prefs.AutoImportFileExcludePatterns, autoImportEntrypointDirectorySearch: prefs.AutoImportEntrypointDirectorySearch}
}
func (p bucketBuildPreferences) Equal(other bucketBuildPreferences) bool {
	return core.UnorderedEqual(p.fileExcludePatterns, other.fileExcludePatterns) && p.autoImportEntrypointDirectorySearch == other.autoImportEntrypointDirectorySearch
}
...
func (b BucketState) possiblyNeedsRebuildForFile(file tspath.Path, preferences lsutil.UserPreferences) bool {
	return b.newProgramStructure > 0 || b.hasDirtyFileBesides(file) || !b.buildPreferences.Equal(bucketBuildPreferencesFromUserPreferences(preferences)) || b.dirtyPackages.Len() > 0
}
...
func newRegistryBucket() *RegistryBucket {
	return &RegistryBucket{state: BucketState{multipleFilesDirty: true, newProgramStructure: newProgramStructureDifferentFileNames}}
}
func (b *RegistryBucket) Clone() *RegistryBucket {
	return &RegistryBucket{state: b.state.Clone(), Paths: b.Paths, PackageFiles: b.PackageFiles, ResolvedPackageNames: b.ResolvedPackageNames, DependencyNames: b.DependencyNames, AmbientModuleNames: b.AmbientModuleNames, Index: b.Index}
}
...
func (d *directory) Clone() *directory {
	return &directory{name: d.name, packageJson: d.packageJson, hasNodeModules: d.hasNodeModules}
}
...
func NewRegistry(toPath func(fileName string) tspath.Path, preferences lsutil.UserPreferences) *Registry {
	return &Registry{toPath: toPath, userPreferences: preferences, directories: make(map[tspath.Path]*directory)}
}
...
func (r *Registry) GetCacheStats() *CacheStats {
	stats := &CacheStats{UniquePackageCount: r.uniquePackageCount}
	for path, bucket := range r.projects {
		exportCount := 0
		if bucket.Index != nil {
			exportCount = len(bucket.Index.entries)
		}
		stats.ProjectBuckets = append(stats.ProjectBuckets, BucketStats{Path: path, ExportCount: exportCount, FileCount: len(bucket.Paths), State: bucket.state, DependencyNames: bucket.DependencyNames, PackageNames: nil})
	}
	for path, bucket := range r.nodeModules {
		...
		stats.NodeModulesBuckets = append(stats.NodeModulesBuckets, BucketStats{Path: path, ExportCount: exportCount, FileCount: fileCount, State: bucket.state, DependencyNames: bucket.DependencyNames, PackageNames: packageNames})
	}
...
func (b *registryBuilder) updateBucketAndDirectoryExistence(change RegistryChange, logger *logging.LogTree) {
	...
	core.DiffMapsFunc(b.base.projects, neededProjects, func(_ *RegistryBucket, _ struct{}) bool {
	...
	updateDirectory := func(dirPath tspath.Path, dirName string, packageJsonChanged bool) {
		...
		if entry, ok := b.directories.Get(dirPath); ok {
			...
			b.directories.Add(dirPath, &directory{name: dirName, packageJson: b.host.GetPackageJson(packageJsonFileName), hasNodeModules: hasNodeModules})
		}
		if hasNodeModules {
	...
	core.DiffMapsFunc(b.base.directories, neededDirectories, func(dir *directory, dirName string) bool {
		...
		for uri := range uris {
			...
			if len(cleanNodeModulesBuckets) > 0 {
				if nodeModulesIndex := strings.Index(string(path), "/node_modules/"); nodeModulesIndex != -1 {
					...
					if _, ok := cleanNodeModulesBuckets[dirPath]; ok {
						...
						entry.Change(func(bucket *RegistryBucket) {
			...
			for projectDirPath := range cleanProjectBuckets {
				...
				if _, ok := entry.Value().Paths[path]; ok {
					entry.Change(func(bucket *RegistryBucket) {
						bucket.markProjectFileDirty(path)
					})
					if !entry.Value().state.multipleFilesDirty {
...
func (b *registryBuilder) updateIndexes(ctx context.Context, change RegistryChange, logger *logging.LogTree) {
	...
	tspath.ForEachAncestorDirectoryPath(change.RequestedFile, func(dirPath tspath.Path) (any, bool) {
		if nodeModulesBucket, ok := b.nodeModules.Get(dirPath); ok {
			...
			needsFullRebuild := bucketState.multipleFilesDirty || !nodeModulesBucket.Value().DependencyNames.Equals(dependencies) || !bucketState.buildPreferences.Equal(bucketBuildPreferencesFromUserPreferences(b.userPreferences)) || !recursiveSearchSubset(targetRecursivePackages, bucketState.recursiveSearchPackages)
			dirtyPackages := bucketState.DirtyPackages()
			canDoGranularUpdate := !needsFullRebuild && dirtyPackages.Len() > 0
			if needsFullRebuild {
				nodeModulesTasks = append(nodeModulesTasks, &nodeModulesBucketTask{entry: nodeModulesBucket, dependencyNames: dependencies, dirName: dirName, dirPath: dirPath})
			} else if canDoGranularUpdate {
				nodeModulesTasks = append(nodeModulesTasks, &nodeModulesBucketTask{entry: nodeModulesBucket, dependencyNames: dependencies, dirName: dirName, dirPath: dirPath, isUpdate: true, existingBucket: nodeModulesBucket.Value(), dirtyPackages: dirtyPackages})
			}
		}
		return nil, false
	})
		...
		for _, pkg := range task.discovered {
			if pkg.realpath != "" {
				if !seen[pkg.realpath] {
					...
					enableDirSearch := targetRecursivePackages == nil || targetRecursivePackages.Has(pkg.packageName) || knownRecursiveSearchPackages.Has(pkg.packageName)
				...
				if !seen[pkg.typesRealpath] {
					...
					wg.Go(func() {
						...
						result := b.extractPackage(ctx, pkg.typesPackageJson, pkg.packageName, projectReferenceOutputs, fileExcludePatterns, true)
						if result != nil {
							extractionMu.Lock()
	...
	for _, task := range nodeModulesTasks {
		...
		wg.Go(func() {
			if task.isUpdate {
				b.updateNodeModulesBucket(ctx, br, task.existingBucket, task.dirtyPackages, task.discovered, extractionCache, targetRecursivePackages, nodeModulesLogger.Fork(task.dirName))
			} else {
				b.buildNodeModulesBucket(ctx, br, task.dependencyNames, task.dirPath, task.discovered, task.directoryPackageNames, extractionCache, targetRecursivePackages, nodeModulesLogger.Fork(task.dirName))
			}
		})
	}
	if project, hasProject := b.projects.Get(projectPath); hasProject {
		...
		shouldRebuild := project.Value().state.hasDirtyFileBesides(change.RequestedFile) || !project.Value().state.buildPreferences.Equal(bucketBuildPreferencesFromUserPreferences(b.userPreferences))
		if !shouldRebuild && project.Value().state.newProgramStructure > 0 {
			if !project.Value().ResolvedPackageNames.Equals(resolvedPackageNames) || hasNewNonNodeModulesFiles(program, project.Value()) {
				...
				project.Change(func(b *RegistryBucket) {
					b.state.newProgramStructure = newProgramStructureFalse
				})
			}
		}
		if shouldRebuild {
			...
			allResults = append(allResults, br)
			wg.Go(func() {
				b.buildProjectBucket(ctx, br, projectPath, resolvedPackageNames, logger.Fork("Building project bucket "+string(projectPath)))
			})
		}
	}
	wg.Wait()
	...
	for _, br := range allResults {
		...
		if len(rootFiles) > 0 {
			...
			aliasResolver := newAliasResolver(slices.Collect(maps.Values(rootFiles)), nil, b.host, moduleResolver, b.base.toPath, func(_ ast.HasFileName, _ string) {
			})
...
func (b *registryBuilder) buildProjectBucket(ctx context.Context, result *bucketBuildResult, projectPath tspath.Path, resolvedPackageNames *collections.Set[string], logger *logging.LogTree) {
	if ctx.Err() != nil {
		result.err = ctx.Err()
		return
	}
	...
	for _, file := range program.GetSourceFiles() {
		...
		if file.ContentMapper() == "" && (strings.Contains(file.FileName(), "/node_modules/") || hasSymlinkToNodeModules(file.Path(), projectRootPath, symlinkCache)) {
			continue
		}
		wg.Go(func() {
			if ctx.Err() == nil {
...
func (b *registryBuilder) discoverBucketPackages(packageNames *collections.Set[string], dirName string, dirPath tspath.Path) []*discoveredPackage {
	...
	for packageName := range packageNames.Keys() {
		...
		isLocal := realpath != "" && !strings.Contains(realpath, "/node_modules/") && tspath.ContainsPath(b.host.GetCurrentDirectory(), realpath, tspath.ComparePathsOptions{UseCaseSensitiveFileNames: b.host.FS().UseCaseSensitiveFileNames()})
		result = append(result, &discoveredPackage{packageName: packageName, packageJson: packageJson, realpath: realpath, typesPackageJson: typesPackageJson, typesRealpath: typesRealpath, dirPath: dirPath, isLocal: isLocal})
	}
	return result
}

func (b *registryBuilder) extractPackage(ctx context.Context, packageJson *packagejson.InfoCacheEntry, packageName string, projectReferenceOutputs map[tspath.Path]string, fileExcludePatterns *vfsmatch.SpecMatcher, enableDirectorySearch bool) *perPackageExtractionResult {
	...
	result := &perPackageExtractionResult{packageFiles: make(map[tspath.Path]string), entrypoints: packageEntrypoints, exports: make(map[tspath.Path][]*Export), ambientModules: make(map[string][]string), skippedEntrypoints: skippedEntrypoints, failedAmbientModuleLookupSources: make(map[tspath.Path]*failedAmbientModuleLookupSource), failedAmbientModuleLookupTargets: &collections.Set[string]{}}
	...
	aliasResolver := newAliasResolver(rootFiles, symlinks, b.host, resolver, b.base.toPath, func(source ast.HasFileName, moduleName string) {
		...
		if _, exists := result.failedAmbientModuleLookupSources[source.Path()]; !exists {
			result.failedAmbientModuleLookupSources[source.Path()] = &failedAmbientModuleLookupSource{fileName: source.FileName()}
		}
	})
	ch, _ := checker.NewChecker(aliasResolver, nil)
...
func (b *registryBuilder) buildNodeModulesBucket(ctx context.Context, result *bucketBuildResult, dependencies *collections.Set[string], dirPath tspath.Path, discovered []*discoveredPackage, directoryPackageNames *collections.Set[string], extractionCache map[string]*perPackageExtractionResult, recursiveSearchPackages *collections.Set[string], logger *logging.LogTree) {
	...
	result.bucket = &RegistryBucket{Index: &Index[*Export]{}, DependencyNames: dependencies, PackageFiles: allPackageFiles, AmbientModuleNames: extraction.ambientModuleNames, Paths: paths, state: BucketState{buildPreferences: bucketBuildPreferencesFromUserPreferences(b.userPreferences), recursiveSearchPackages: recursiveSearchPackages.Clone()}}
...
func (b *registryBuilder) updateNodeModulesBucket(ctx context.Context, result *bucketBuildResult, existingBucket *RegistryBucket, dirtyPackages *collections.Set[string], discovered []*discoveredPackage, extractionCache map[string]*perPackageExtractionResult, recursiveSearchPackages *collections.Set[string], logger *logging.LogTree) {
	if ctx.Err() != nil {
		result.err = ctx.Err()
		return
	}
	...
	result.bucket = &RegistryBucket{Index: newIndex, DependencyNames: existingBucket.DependencyNames, PackageFiles: newPackageFiles, AmbientModuleNames: newAmbientModuleNames, Paths: newPaths, state: BucketState{buildPreferences: bucketBuildPreferencesFromUserPreferences(b.userPreferences), recursiveSearchPackages: recursiveSearchPackages.Clone()}}
