...
func (l *LanguageService) GetEditsForFileRename(ctx context.Context, oldURI lsproto.DocumentUri, newURI lsproto.DocumentUri) []lsproto.TextDocumentEditOrCreateFileOrRenameFileOrDeleteFile {
		...
		for _, ext := range originalExtensions {
			...
			if l.host.FileExists(oldOriginalPath) {
				...
				if slices.Contains(newOriginalExtensions, ext) {
					...
					documentChanges = append(documentChanges, lsproto.TextDocumentEditOrCreateFileOrRenameFileOrDeleteFile{RenameFile: &lsproto.RenameFile{OldUri: lsconv.FileNameToDocumentURI(oldOriginalPath), NewUri: lsconv.FileNameToDocumentURI(newOriginalPath)}})
				}
			}
		}
	}
	...
	for fileName, edits := range changes {
		...
		for _, edit := range edits {
			lspEdits = append(lspEdits, lsproto.TextEditOrAnnotatedTextEditOrSnippetTextEdit{TextEdit: edit})
		}
		documentChanges = append(documentChanges, lsproto.TextDocumentEditOrCreateFileOrRenameFileOrDeleteFile{TextDocumentEdit: &lsproto.TextDocumentEdit{TextDocument: lsproto.OptionalVersionedTextDocumentIdentifier{Uri: uri}, Edits: lspEdits}})
	}
...
func (l *LanguageService) createPathUpdater(oldPath string, newPath string) pathUpdater {
	...
	return func(path string) (string, bool) {
		...
		if suffix, ok := tspath.TrimFilePathPrefix(path, trimmedOldPath, l.UseCaseSensitiveFileNames()); ok && (strings.HasPrefix(suffix, "/") || strings.HasPrefix(suffix, "\\")) {
...
func (l *LanguageService) updateTsconfigFiles(program *compiler.Program, changeTracker *change.Tracker, oldToNew pathUpdater, oldPath string, newPath string) {
	...
	forEachObjectProperty(jsonObjectLiteral, func(property ast.Handle, propertyName string) {
		switch propertyName {
		case "files", "include", "exclude":
			...
			if oldSpec, isDefault := commandLine.GetMatchedIncludeSpec(oldPath); oldSpec != "" && !isDefault {
				if newSpec, _ := commandLine.GetMatchedIncludeSpec(newPath); newSpec == "" {
					...
					if len(elements) > 0 {
						changeTracker.InsertNodeAfter(configFile, elements[len(elements)-1], changeTracker.HandleFactory.NewStringLiteral(relativePathFromDirectory(configDir, newPath, l.UseCaseSensitiveFileNames()), ast.TokenFlagsNone))
					}
				}
			}
		case "compilerOptions":
			...
			forEachObjectProperty(property.Initializer(), func(property ast.Handle, propertyName string) {
				...
				forEachObjectProperty(property.Initializer(), func(pathsProperty ast.Handle, _ string) {
...
func updatePathsProperty(configFile *ast.SourceFile, configDir string, property ast.Handle, changeTracker *change.Tracker, oldToNew pathUpdater, converters *lsconv.Converters, useCaseSensitiveFileNames bool) bool {
	elements := []ast.Handle{property.Initializer()}
...
func tryUpdateConfigString(configFile *ast.SourceFile, configDir string, element ast.Handle, changeTracker *change.Tracker, oldToNew pathUpdater, converters *lsconv.Converters, useCaseSensitiveFileNames bool) bool {
...
func (l *LanguageService) getUpdatedImportSpecifier(program *compiler.Program, checker *checker.Checker, sourceFile *ast.SourceFile, importLiteral ast.Handle, oldToNew pathUpdater, movedFiles []movedFile, newImportFromPath string, importingSourceFileMoved bool, userPreferences modulespecifiers.UserPreferences) string {
	...
	updated := modulespecifiers.UpdateModuleSpecifier(program.Options(), program, sourceFile, newImportFromPath, importLiteral.Text(), target.newFileName, userPreferences, modulespecifiers.ModuleSpecifierOptions{OverrideImportMode: program.GetModeForUsageLocation(sourceFile, importLiteral)})
	return updated
}
func getSourceFileToImport(program *compiler.Program, sourceFile *ast.SourceFile, importLiteral ast.Handle, oldToNew pathUpdater) *toImport {
...
func getUpdatedImportSpecifierFromMovedSourceFiles(program *compiler.Program, sourceFile *ast.SourceFile, importLiteral ast.Handle, movedFiles []movedFile, importingSourceFileName string, userPreferences modulespecifiers.UserPreferences) string {
	resolutionMode := program.GetModeForUsageLocation(sourceFile, importLiteral)
	for _, candidate := range movedFiles {
		oldSpecifier := modulespecifiers.UpdateModuleSpecifier(program.Options(), program, sourceFile, importingSourceFileName, importLiteral.Text(), candidate.sourceFile.FileName(), userPreferences, modulespecifiers.ModuleSpecifierOptions{OverrideImportMode: resolutionMode})
		if oldSpecifier != importLiteral.Text() {
			continue
		}
		return modulespecifiers.UpdateModuleSpecifier(program.Options(), program, sourceFile, importingSourceFileName, importLiteral.Text(), candidate.newFileName, userPreferences, modulespecifiers.ModuleSpecifierOptions{OverrideImportMode: resolutionMode})
	}
	return ""
}
func createStringTextRange(sourceFile *ast.SourceFile, node ast.Handle) core.TextRange {
...
func forEachObjectProperty(objectLiteral ast.Handle, cb func(property ast.Handle, propertyName string)) {
