...
var importFixErrorCodes = []int32{// ImportFixProvider is the CodeFixProvider for import-related fixes
...
/* excludeGlobals */ // Compute usage position for JSDoc import type fixes
...
/* excludeGlobals */ // Get the type-only alias declaration
// whether the symbol currently resolves to a type-only import
/* excludeGlobals */ /* excludeGlobals */ /* excludeGlobals */ /* excludeGlobals */ // Create a copy to avoid modifying the original
// Sort by:
...
var ImportFixProvider = &CodeFixProvider{ErrorCodes: importFixErrorCodes, GetCodeActions: getImportCodeActions, FixIds: []string{importFixID}, GetAllCodeActions: getAllImportCodeActions}
...
func getImportCodeActions(ctx context.Context, fixContext *CodeFixContext) ([]*CodeAction, error) {
	...
	for _, fixInfo := range info {
		edits, description, ok := fixInfo.fix.Edits(ctx, fixContext.SourceFile, fixContext.Program.Options(), fixContext.LS.FormatOptions(), fixContext.LS.converters, fixContext.LS.UserPreferences())
		if ok {
			actions = append(actions, &CodeAction{Description: description, Changes: edits, FixID: importFixID, FixAllDescription: diagnostics.Add_all_missing_imports.Localize(locale.FromContext(ctx))})
		}
	}
	return actions, nil
}
func getAllImportCodeActions(ctx context.Context, fixContext *CodeFixContext) (*CombinedCodeActions, error) {
	...
	defer done()
	importAdder := autoimport.NewImportAdder(ctx, fixContext.Program, ch, fixContext.SourceFile, view, fixContext.LS.FormatOptions(), fixContext.LS.converters, fixContext.LS.UserPreferences())
	...
	return &CombinedCodeActions{Description: diagnostics.Add_all_missing_imports.Localize(locale.FromContext(ctx)), Changes: importAdder.Edits()}, nil
}
...
func addImportFromDiagnostic(ctx context.Context, importAdder autoimport.ImportAdder, diag *ast.Diagnostic, fixContext *CodeFixContext) error {
	diagFixContext := &CodeFixContext{SourceFile: fixContext.SourceFile, Span: core.NewTextRange(diag.Pos(), diag.End()), ErrorCode: diag.Code(), Program: fixContext.Program, LS: fixContext.LS}
...
func getFixesInfoForUMDImport(ctx context.Context, fixContext *CodeFixContext, token ast.Handle, view *autoimport.View) []*fixInfo {
	ch, done := fixContext.Program.GetTypeChecker(ctx)
	defer done()
	...
	for _, fix := range view.GetFixes(ctx, export, false, isValidTypeOnlyUseSite, nil) {
		...
		result = append(result, &fixInfo{fix: fix, symbolName: umdSymbol.Name, errorIdentifierText: errorIdentifierText})
	}
	return result
}
func getUmdSymbol(token ast.Handle, ch *checker.Checker) *ast.Symbol {
	...
	if (ast.IsJsxOpeningLikeElement(parent) && parent.TagName() == token) || ast.IsJsxOpeningFragment(parent) {
		...
		parentSymbol := ch.ResolveName(jsxNamespace, location, ast.SymbolFlagsValue, false)
	...
	for _, sn := range symbolNames {
		...
		for _, export := range exports {
			...
			for _, fix := range fixes {
				allInfo = append(allInfo, &fixInfo{fix: fix, symbolName: symbolName, isJsxNamespaceFix: symbolName != symbolToken.Text()})
			}
		}
	}
	return allInfo
}
func getTypeOnlyPromotionFix(ctx context.Context, sourceFile *ast.SourceFile, symbolToken ast.Handle, symbolName string, program *compiler.Program) *autoimport.Fix {
	ch, done := program.GetTypeChecker(ctx)
	defer done()
	symbol := ch.ResolveName(symbolName, symbolToken, ast.SymbolFlagsValue, true)
	if symbol == nil {
		return nil
	}
	...
	return &autoimport.Fix{AutoImportFix: &lsproto.AutoImportFix{Kind: lsproto.AutoImportFixKindPromoteTypeOnly}, TypeOnlyAliasDeclaration: typeOnlyAliasDeclaration}
}
...
func getSymbolNamesToImport(sourceFile *ast.SourceFile, ch *checker.Checker, symbolToken ast.Handle, compilerOptions *core.CompilerOptions) []symbolNameInfo {
	...
	if (ast.IsJsxOpeningLikeElement(parent) || ast.IsJsxClosingElement(parent)) && parent.TagName() == symbolToken && jsxModeNeedsExplicitImport(compilerOptions.Jsx) {
		jsxNamespace := ch.GetJsxNamespace(sourceFile.ParseRoot())
		if needsJsxNamespaceFix(jsxNamespace, symbolToken, ch) {
			...
			if !scanner.IsIntrinsicJsxName(symbolToken.Text()) {
				compSymbol := ch.ResolveName(symbolToken.Text(), symbolToken, ast.SymbolFlagsValue, false)
				if compSymbol == nil {
			...
			nsIsTypeOnly := false
			if nsSymbol := ch.ResolveName(jsxNamespace, symbolToken, ast.SymbolFlagsValue, true); nsSymbol != nil {
	...
	if sym := ch.ResolveName(symbolToken.Text(), symbolToken, ast.SymbolFlagsValue, true); sym != nil {
...
func needsJsxNamespaceFix(jsxNamespace string, symbolToken ast.Handle, ch *checker.Checker) bool {
	if scanner.IsIntrinsicJsxName(symbolToken.Text()) {
		return true
	}
	namespaceSymbol := ch.ResolveName(jsxNamespace, symbolToken, ast.SymbolFlagsValue, true)
