...
func programToSnapshot(program *compiler.Program, oldProgram *Program, hashWithText bool) *snapshot {
	...
	snapshot := &snapshot{options: program.Options(), hashWithText: hashWithText, checkPending: program.Options().NoCheck.IsTrue()}
	to := &toProgramSnapshot{program: program, oldProgram: oldProgram, snapshot: snapshot}
...
func (t *toProgramSnapshot) reuseFromOldProgram( // Copy old snapshot's changed files set
...
func (t *toProgramSnapshot) computeProgramFileChanges() {
	canCopySemanticDiagnostics := t.oldProgram != nil && !tsoptions.CompilerOptionsAffectSemanticDiagnostics(t.oldProgram.snapshot.options, t.program.Options())
	canCopyEmitSignatures := t.snapshot.options.Composite.IsTrue() && t.oldProgram != nil && !tsoptions.CompilerOptionsAffectDeclarationPath(t.oldProgram.snapshot.options, t.program.Options())
	copyDeclarationFileDiagnostics := canCopySemanticDiagnostics && t.snapshot.options.SkipLibCheck.IsTrue() == t.oldProgram.snapshot.options.SkipLibCheck.IsTrue()
	copyLibFileDiagnostics := copyDeclarationFileDiagnostics && t.snapshot.options.SkipDefaultLibCheck.IsTrue() == t.oldProgram.snapshot.options.SkipDefaultLibCheck.IsTrue()
	...
	for _, file := range files {
		wg.Queue(func() {
			...
			t.snapshot.fileInfos.Store(file.Path(), &FileInfo{version: version, signature: signature, affectsGlobalScope: affectsGlobalScope, impliedNodeFormat: impliedNodeFormat})
		})
	}
	wg.RunAndWait()
}
...
func (t *toProgramSnapshot) handlePendingCheck() {
	if t.oldProgram != nil && t.snapshot.semanticDiagnosticsPerFile.Size() != len(t.program.GetSourceFiles()) && t.oldProgram.snapshot.checkPending != t.snapshot.checkPending {
		t.snapshot.buildInfoEmitPending.Store(true)
	}
}
func fileAffectsGlobalScope(file *ast.SourceFile) bool {
	binder.BindSourceFile(file)
	if core.Some(file.ModuleAugmentations, func(augmentation ast.Handle) bool {
	...
	return core.Some(file.ParseRoot().Statements(), func(stmt ast.Handle) bool {
...
func addReferencedFilesFromImportLiteral(file *ast.SourceFile, referencedFiles *collections.Set[tspath.Path], checker *checker.Checker, importName ast.Handle) {
...
func repopulateDiagnosticMessageChain(chain []*ast.Diagnostic, p *compiler.Program, file *ast.SourceFile) []*ast.Diagnostic {
	...
	for i, c := range chain {
		if c.RepopulateInfo() != nil {
			b := &buildInfoDiagnosticWithFileName{pos: c.Pos(), end: c.End(), code: c.Code(), category: c.Category(), source: c.Source(), messageText: c.MessageText(), messageKey: c.MessageKey(), messageArgs: c.MessageArgs(), repopulateInfo: c.RepopulateInfo()}
...
func astDiagToBuildInfoDiag(d *ast.Diagnostic) *buildInfoDiagnosticWithFileName {
	b := &buildInfoDiagnosticWithFileName{pos: d.Pos(), end: d.End(), code: d.Code(), category: d.Category(), source: d.Source(), messageText: d.MessageText(), messageKey: d.MessageKey(), messageArgs: d.MessageArgs(), repopulateInfo: d.RepopulateInfo()}
